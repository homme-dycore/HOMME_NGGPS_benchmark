...
! Neighborhood collectives are only in MPI3 and up
   ...
   if(ithr == 0) then 
      ...
      call t_startf('bndry_a2a', t_detail_medium)
      ...
      call t_startf('bndry_copy', t_detail_high)
      call copyBuffer(nthreads,ithr,buffer,locstring)
      call t_stopf('bndry_copy', t_detail_high)

      call MPI_wait(request,lstatus,ierr)
      call t_stopf('bndry_a2a', t_detail_medium)
   else
...
!
   ...
   call t_startf('bndry_put', t_detail_medium)
   if(ithr == 0) then 
...
!       if(Debug) print *,'IAM: ', iam, 'After MPI_Get'
   ...
   call t_stopf('bndry_put', t_detail_medium)
...
!
   ...
   pSchedule => schedule(1)
   call t_startf('bndry_get', t_detail_medium)
   if(ithr == 0) then 
...
!    call MPI_win_start(par%commGraphfull,0,buffer%win,ierr)
   ...
   call t_stopf('bndry_get', t_detail_medium)
...
! Neighborhood collectives are only in MPI3 and up
   ...
   if(ithr == 0) then 
      ...
      call t_startf('bndry_a2ao', t_detail_medium)
      ...
      call t_stopf('bndry_a2ao', t_detail_medium)
...
!    call t_adj_detailf(+2)
    call t_startf(subname, t_detail_medium)
    ...
    call gbarrier(buffer%gbarrier, hybrid%ithr)
    call t_stopf(subname, t_detail_medium)
!    call t_adj_detailf(-2)
