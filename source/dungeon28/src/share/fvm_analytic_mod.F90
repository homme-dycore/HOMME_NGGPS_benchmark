...
module fvm_analytic_mod
  ...
  use dimensions_mod, only: nc, nhe, ntrac, pr_dbg
...
subroutine computexytosphere_moments(fvm,desc)
  ...
  real (kind=real_kind)                               :: tmpval(1-nhe:nc+nhe,MAX(irecons_tracer, irecons_air)-1)
...
subroutine moment_onsphere(area,centroid,x,y,jx_min,jx_max,jy_min,jy_max)
  ...
  real (kind=real_kind), dimension(1-nhe:nc+nhe,1-nhe:nc+nhe,MAX(irecons_tracer, irecons_air)-1), intent(inout) :: centroid
  real (kind=real_kind), dimension(-nhe:nc+2+nhe), intent(in)   :: x, y
  integer, intent(in)                                  :: jx_min,jx_max,jy_min,jy_max
  integer                                              :: i,j

  select case (MAX(irecons_tracer, irecons_air))
  case(3)
  ...
  case default
     write(*,*) "irecons out of range",irecons_tracer,irecons_air
     stop
  end select
  ...
  case(3)
     do j=jy_min,jy_max-1
        do i=jx_min,jx_max-1
           ...
           ! Compute centroids via line integrals
           centroid(i,j,1) = (I_10(y(j+1),x(i+1)) - I_10(y(j+1),x(i)) + &
                I_10(y(j),x(i)) - I_10(y(j),x(i+1))) / area(i,j)
           centroid(i,j,2) = (I_01(y(j+1),x(i+1)) - I_01(y(j+1),x(i)) + &
  ...
  case(6)
     do j=jy_min,jy_max-1
        do i=jx_min,jx_max-1
           ...
           !ADD PHL START
           ! TAN(alpha)^2 component
           centroid(i,j,3) = (I_20(y(j+1),x(i+1)) - I_20(y(j+1),x(i)) + &
                I_20(y(j),x(i)) - I_20(y(j),x(i+1))) / area(i,j)
           ! TAN(beta)^2 component
           centroid(i,j,4) = (I_02(y(j+1),x(i+1)) - I_02(y(j+1),x(i)) + &
                I_02(y(j),x(i)) - I_02(y(j),x(i+1))) / area(i,j)
           ! TAN(alpha) TAN(beta) component
           centroid(i,j,5) = (I_11(y(j+1),x(i+1)) - I_11(y(j+1),x(i)) + &
...
subroutine compute_reconstruct_matrix(fvm)
  ...
  real (kind=real_kind) :: coef, coef2, tmp, cartx,carty
  !
  ...
  case(3)
     do j= 1-nhe,nc+nhe
        do i=1-nhe,nc+nhe
           ...
           coef = coef /( 1.0D0 + fvm%spherecentroid(i,j,1)**2) !stretching coefficient
           
           fvm%centroid_stretch(i,j,1) = coef
           !
           !***************
           !*    dfdy     *
           !***************
           ...
           coef = coef /( 1.0D0 + fvm%spherecentroid(i,j,2)**2) !stretching coefficient
           
           fvm%centroid_stretch(i,j,2) = coef
        end do
     end do
  case(6)
     !
     ...
     do j= 1-nhe,nc+nhe
        do i=1-nhe,nc+nhe
           ...
           !
           tmp = 0.5D0/((1.0D0 + fvm%spherecentroid(i,j,1)**2)**2)
           
           fvm%centroid_stretch(i,j,3) = coef*tmp
           fvm%centroid_stretch(i,j,6) = -fvm%spherecentroid(i,j,1)/(1.0D0 + fvm%spherecentroid(i,j,1)**2)
           ...
           !
           tmp =0.5D0/((1.0D0 + fvm%spherecentroid(i,j,2)**2)**2)
           
           fvm%centroid_stretch(i,j,4) = coef*tmp
           fvm%centroid_stretch(i,j,7) = -fvm%spherecentroid(i,j,2)/(1.0D0 + fvm%spherecentroid(i,j,2)**2)
           !
           !*****************
           !*    d2fdxdy    *
           !*****************
           ...
           coef = coef  / ((1.0D0 + fvm%spherecentroid(i,j,1)**2) * &
                (1.0D0 + fvm%spherecentroid(i,j,2)**2))    !stretching coefficient 
           
           fvm%centroid_stretch(i,j,5) = coef
        enddo
     enddo
  case default
     write(*,*) "irecons out of range",irecons_tracer,irecons_air
