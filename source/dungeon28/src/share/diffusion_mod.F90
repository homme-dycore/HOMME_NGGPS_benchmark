...
contains
  ...
  subroutine prim_diffusion(elem, nets,nete,np1,deriv,dt2,hybrid)
    ...
    call t_startf('prim_diffusion', t_detail_high)
...
!$omp parallel do num_threads(vert_num_threads), private(grad_tmp,v1,v2,gp,vco,div_tmp,zeta_tmp)
#endif
       do k=1,nlev   
          call gradient_routine(elem(ie)%state%T(:,:,k,np1),deriv,grad_tmp(:,:,:))
...
! Map grad_tmp to lat-lon instead of rotating it
          ...
          call divergence_routine(gp,deriv,div_tmp(:,:))
          div_tmp(:,:)=div_tmp(:,:)*rrearth

          call vorticity_routine(vco,deriv,zeta_tmp(:,:))
    ...
    call t_startf('bndry_exchangeV.edge4', t_detail_medium)
    call bndry_exchangeV(hybrid,edge4)
    call t_stopf('bndry_exchangeV.edge4', t_detail_medium)
...
#if (defined _LOOP_OPENMP)
!$omp parallel do num_threads(vert_num_threads), private(grad_tmp,div_tmp,grad_div,grad_zeta,v1,v2)
#endif
       do k=1,nlev
          do j=1,np
             do i=1,np
...
!                grad_T_np1(i,j,2,k,ie) = metdet(i,j)*(metinv(i,j,2,1)*v1 + metinv(i,j,2,2)*v2)
       ...
       (elem(ie)%Dinv(i,j,2,1)*grad_tmp(i,j,1)+elem(ie)%Dinv(i,j,2,2)*grad_tmp(i,j,2))
          ...
          call divergence_routine(grad_T_np1(:,:,:,k,ie),deriv,div_tmp(:,:))
          div_tmp(:,:)=div_tmp(:,:)*rrearth
          ...
          call gradient_routine(div_np1(:,:,k,ie),deriv,grad_div(:,:,:))
          grad_div(:,:,:)=grad_div(:,:,:)*rrearth

          call gradient_routine(zeta_np1(:,:,k,ie),deriv,grad_zeta(:,:,:))
...
!$OMP BARRIER
#endif
    call t_stopf('prim_diffusion', t_detail_high)
  ...
  subroutine scalar_diffusion(elem, nets,nete,np1,deriv,dt2,hybrid)
    ...
    call t_startf('scalar_diffusion', t_detail_high)
    do ie=nets,nete
       do q=1,qsize
...
#if (defined _LOOP_OPENMP)
!$omp parallel do num_threads(vert_num_threads), private(grad_tmp,v1,v2)
#endif
          do k=1,nlev   
            
             call gradient_routine(elem(ie)%state%Q(:,:,k,q),deriv,grad_tmp(:,:,:))
             grad_tmp(:,:,:)=grad_tmp(:,:,:)*rrearth 
    ...
    call t_startf('bndry_exchangeV.edgeS2', t_detail_medium)
    call bndry_exchangeV(hybrid,edgeS2)
    call t_stopf('bndry_exchangeV.edgeS2', t_detail_medium)
...
#if (defined _LOOP_OPENMP)
!$omp parallel do num_threads(vert_num_threads), private(div_tmp)
#endif
          do k=1,nlev
             do j=1,np
                do i=1,np
             ...
             call divergence_routine(grad_Q_np1(:,:,:,k,q,ie),deriv,div_tmp(:,:))
             div_tmp(:,:)=div_tmp(:,:)*rrearth
    ...
    call t_startf('bndry_exchangeV.edgeS1', t_detail_medium)
    call bndry_exchangeV(hybrid,edgeS1)
    call t_stopf('bndry_exchangeV.edgeS1', t_detail_medium)
    do ie=nets,nete
...
!$OMP BARRIER
#endif
    call t_stopf('scalar_diffusion', t_detail_high)
