...
!
    ...
    use derivative_mod, only : curl_sphere_routine 
...
#define IBYP_TEST
    ...
    do ie=nets,nete
       ...
       call curl_sphere_routine(E(:,:,ie),deriv,elem(ie),grade)
       call divergence_sphere_routine(grade,deriv,elem(ie),div)
       call vorticity_sphere_routine(grade,deriv,elem(ie),vor)
       ...
       call gradient_sphere_routine(E(:,:,ie),deriv,elem(ie)%Dinv,grade)
       call vorticity_sphere_routine(grade,deriv,elem(ie),vor)
       call divergence_sphere_routine(grade,deriv,elem(ie),div)
    ...
    do ie=nets,nete
       call curl_sphere_routine(E(:,:,ie),deriv,elem(ie),gradbig(:,:,:,ie))
    enddo
    call make_C0_vector(gradbig,elem,hybrid,nets,nete)
    do ie=nets,nete
       call divergence_sphere_routine(gradbig(:,:,:,ie),deriv,elem(ie),divbig(:,:,ie))
    enddo
    ...
    do ie=nets,nete
       call gradient_sphere_routine(E(:,:,ie),deriv,elem(ie)%Dinv,gradbig(:,:,:,ie))
    enddo
    call make_C0_vector(gradbig,elem,hybrid,nets,nete)
    do ie=nets,nete
       call vorticity_sphere_routine(gradbig(:,:,:,ie),deriv,elem(ie),divbig(:,:,ie))
    enddo
    call make_C0(divbig,elem,hybrid,nets,nete)
    ...
    do ie=nets,nete
       ...
       call divergence_sphere_routine(pv(1,1,1,ie),deriv,elem(ie),div)      ! latlon vector -> scalar 
       call gradient_sphere_routine(E(1,1,ie),deriv,elem(ie)%Dinv,grade)
       call divergence_sphere_wk_routine(pv(1,1,1,ie),deriv,elem(ie),wdiv) 
...
!$OMP BARRIER
  ...
  subroutine test_subcell_div_fluxes(elem,deriv,nets,nete)
    ...
    do ie=nets,nete
      ...
      call divergence_sphere_routine(u, deriv, elem(ie),div)
  ...
  subroutine test_subcell_Laplace_fluxes(elem,deriv,nets,nete)
    ...
    use derivative_mod, only : subcell_Laplace_fluxes
    use derivative_mod, only : subcell_integration
    use derivative_mod, only : laplace_sphere_wk_routine, gradient_sphere_routine
    use derivative_mod, only : divergence_sphere_routine, divergence_sphere_wk_routine 
    use element_mod,    only : element_t
    use derivative_mod, only : derivative_t
    use kinds,          only : real_kind
    use coordinate_systems_mod, only: spherical_polar_t
    ...
    do ie=nets,nete
      ...
      call laplace_sphere_wk_routine(u,deriv,elem(ie),var_coef=.false.,laplace=laplace)
...
!
  ...
  use element_mod, only    : element_t
  use derivative_mod, only : derivative_t, divergence_sphere_routine, divergence_sphere_wk_routine
  use derivative_mod, only : element_boundary_integral, gradient_sphere_routine
  use derivative_mod, only : gradient_sphere_wk_testcontra_routine, gradient_sphere_wk_testcov_routine
  use derivative_mod, only : curl_sphere_routine, curl_sphere_wk_testcov_routine 
  ...
  do ie=nets,nete
     ...
     call divergence_sphere_routine(ulatlon,deriv,elem(ie),phidivu)
     phidivu = elem(ie)%spheremp(:,:)*phidivu
     call divergence_sphere_wk_routine(ulatlon,deriv,elem(ie),ugradphi)
  ...
  do ie=nets,nete
     ...
     call gradient_sphere_routine(p,deriv,elem(ie)%Dinv,gradp)
     gradp(:,:,1)=gradp(:,:,1)*elem(ie)%spheremp(:,:)  
     gradp(:,:,2)=gradp(:,:,2)*elem(ie)%spheremp(:,:)
     call gradient_sphere_wk_testcontra_routine(p,deriv,elem(ie),gradp_wk)
  ...
  do ie=nets,nete
     ...
     gradp(:,:,1)=gradp(:,:,1)*elem(ie)%spheremp(:,:)  
     gradp(:,:,2)=gradp(:,:,2)*elem(ie)%spheremp(:,:)
     call gradient_sphere_wk_testcov_routine(p,deriv,elem(ie),gradp_wk)
     lhs = gradp(:,:,1)-gradp_wk(:,:,1)
     lhs2 = gradp(:,:,2)-gradp_wk(:,:,2)  
  ...
  do ie=nets,nete
     ...
     ! grad(p)  (lat/lon vector)
     call curl_sphere_routine(p,deriv,elem(ie),gradp)
     gradp(:,:,1)=gradp(:,:,1)*elem(ie)%spheremp(:,:)  
     gradp(:,:,2)=gradp(:,:,2)*elem(ie)%spheremp(:,:)
     call curl_sphere_wk_testcov_routine(p,deriv,elem(ie),gradp_wk)
