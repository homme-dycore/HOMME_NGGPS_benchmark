...
contains
  ...
  function taylor_filter_create(Tp, mu, gquadp) result(flt)
    ...
    call t_startf('taylor_filter_create', t_detail_high)
    allocate(gammaP(np))
    ...
    deallocate(AP)
    deallocate(AinvP)
    call t_stopf('taylor_filter_create', t_detail_high)
  ...
  function fm_filter_create(Tp, mu, gquadP) result(flt)
    ...
    call t_startf('fm_filter_create', t_detail_high)
    ...
    deallocate(legP)
    deallocate(AP)
    call t_stopf('fm_filter_create', t_detail_high)
  ...
  function fm_transfer(kcut,wght,npts) result(T)
    ...
    call t_startf('fm_transfer', t_detail_high)
    kc  = npts-kcut
    cut = kcut
    ...
    call t_stopf('fm_transfer', t_detail_high)
  ...
  function bv_transfer(p,s,npts) result(T)
    ...
    call t_startf('bv_transfer', t_detail_high)
    do k=1,npts
    ...
    call t_stopf('bv_transfer', t_detail_high)
  ...
  function bvsigma(p,x) result(sigma)
    ...
    call t_startf('bvsigma', t_detail_high)
    ...
    call t_stopf('bvsigma', t_detail_high)
...
!DIR$ UNROLL(NP)
  ...
  subroutine preq_filter(elem, edge3p1, flt, hybrid, nfilt, nets, nete)
    ...
    call t_startf('preq_filter', t_detail_high)
    if (flt%type == "bv") then
       do ie=nets,nete          
...
!$OMP BARRIER
    ...
    call t_stopf('preq_filter', t_detail_high)
  ...
  subroutine prim_filter(elem, edge_3lp1,flt,nfilt,nets,nete,hybrid)
    ...
    call t_startf('prim_filter', t_detail_high)
...
!$omp parallel do num_threads(vert_num_threads)
       ...
       call t_startf('bndry_exchangeV.edge_3lp1', t_detail_medium)
       call bndry_exchangeV(hybrid,edge_3lp1,location='prim_filter')
       call t_stopf('bndry_exchangeV.edge_3lp1', t_detail_medium)
...
#endif
#if (defined _LOOP_OPENMP)
!$omp parallel do num_threads(vert_num_threads), private(v1,v2,u,v)
#endif
          do k=1,nlev
             call filter_P(elem(ie)%state%T(:,:,k,nfilt),flt)
       if( integration == "explicit" ) then
    ...
    call t_stopf('prim_filter', t_detail_high)
