...
!$omp parallel do num_threads(num_threads) private(k,facp,facm)
#endif
    do j=1,np 
...
#if (defined _LOOP_OPENMP)
       num_threads = max(vert_num_threads,tracer_num_threads)
!$omp parallel do num_threads(num_threads) private(ckk,ckl,term)
#endif
       do j=1,np 
...
#if (defined _LOOP_OPENMP)
      num_threads = max(vert_num_threads,tracer_num_threads)
!$omp parallel do num_threads(num_threads) private(hkk,hkl)
#endif
       do j=1,np 
