...
module prim_driver_mod
...
#ifndef CAM
  use column_types_mod, only : ColumnModel_t, HeldSuarezForcing_t
  ...
  use fvm_mod, only : fvm_init1,fvm_init2, fvm_init_flux, fill_halo_fvm
  ...
  public :: prim_commtest
  public :: smooth_topo_datasets

  type (cg_t), allocatable  :: cg(:)              ! conjugate gradient struct (max_num_threads)
...
#ifndef CAM
  type (ColumnModel_t), allocatable :: cm(:) ! (max_num_threads)
#endif
...
!    print *,'_init1a: ID: ', iam,' sum(rspheremp): ',sum(elem(1)%rspheremp)
...
!    print *,'before call to second mass_matrix'
    if(par%masterproc) write(iulog,*) 're-running mass_matrix'
    call mass_matrix(par,elem)
!    print *,'_init1b: ID: ', iam,' sum(rspheremp): ',sum(elem(1)%rspheremp)
...
!=======================================================================================================!
  ...
  subroutine prim_init2(elem, fvm, hybrid, nets, nete, tl, hvcoord)
    use dimensions_mod, only: irecons_tracer, irecons_air, qsize_condensate_loading,ldry_mass_vertical_coordinates
    ...
    use prim_state_mod, only : prim_printstate!phlxxx , prim_diag_scalars
...
#ifndef CAM
    use dcmip_wrapper_mod, only: init_dcmip_1_1, init_dcmip_1_2

    use control_mod, only : pertlim                     !used for homme temperature perturbations
#endif
    use prim_si_ref_mod, only: prim_si_refstate_init, prim_set_mass, prim_set_dry_ps
#ifdef TRILINOS
    ...
    use umjs_baroclinic_wave_mod, only : umjs_baroclinic
#endif
...
!    type(element_t)                    :: jac_elem(size(elem))
  ...
  interface
    ...
    if (tstep_type == 0) then
      ...
      call initialize(state_object, tstep_type, elem, hvcoord, &
        qn0, hybrid, deriv, tstep, tl, nets, nete)

      call initialize(pre_object, tstep_type, elem, hvcoord, &
        qn0, hybrid, deriv, tstep, tl, nets, nete)

      call initialize(jac_object, tstep_type, elem, hvcoord, &
        qn0, hybrid, deriv, tstep, tl, nets, nete)
    ...
    if(runtype >= 1) then
       ...
       if (runtype==2) then
          ...
          do ie=nets,nete
             ...
             elem(ie)%state%ps(:,:,tl%nm1)=elem(ie)%state%ps(:,:,tl%n0)
             elem(ie)%state%lnps(:,:,tl%nm1)=elem(ie)%state%lnps(:,:,tl%n0)
          enddo
          print *,'IAM: ',iam, 'sum(v(n0)): ',sum(elem(1)%state%v(:,:,:,:,tl%n0))
          print *,'IAM: ',iam, 'sum(T(n0)): ',sum(elem(1)%state%T(:,:,:,tl%n0))
          print *,'IAM: ',iam, 'sum(ps(n0)): ',sum(elem(1)%state%ps(:,:,tl%n0))
          print *,'IAM: ',iam, 'sum(lnps(n0)): ',sum(elem(1)%state%lnps(:,:,tl%n0))
       endif ! runtype==2
       ...
       else if (test_case(1:15) == "umjs_baroclinic") then
          if (hybrid%masterthread) then
             write(iulog,*) 'initializing Ullrich et al.(2013) baroclinic wave'
          end if
          call umjs_baroclinic(elem, hybrid,hvcoord,nets,nete,fvm,moisture == "dry")
       else if (test_case(1:8) == "DCMIP1-1") then
          if (hybrid%masterthread) then
            write(iulog,*) 'initializing DCMIP test 1-1: 3d deformational flow'
          end if
          call init_dcmip_1_1(elem, hybrid, hvcoord, nets, nete, tl)
       else if (test_case(1:8) == "DCMIP1-2") then
          if (hybrid%masterthread) then
            write(iulog,*) 'initializing DCMIP test 1-2: Hadley-like circulation'
          end if
          call init_dcmip_1_2(elem, hybrid, hvcoord, nets, nete, tl)
       else
       ...
       !
       ! tracer wet mixing ratio => lltrac = .true.
       !
       if (moisture /= "dry") then
          if (.not. ldry_mass_vertical_coordinates) then
             do ie=nets,nete
                do j=1,np
                   do i=1,np
                      elem(ie)%state%ps(i,j,:) = elem(ie)%state%pswet(i,j)
                   end do
                end do
             end do
          else
             !
             ! for idealized tests do not do conversion to dry
             !
             ltrac = .false.
             ltrac(1:qsize_condensate_loading) = .true.
             if (hybrid%masterthread) then
                write(iulog,*) 'Set state to dry mass vertical coordinates'
             end if
             do ie=nets,nete               
                elem(ie)%state%pswet(:,:)=hvcoord%hyai(1)*hvcoord%ps0
                do k=1,nlev
                   do i=1,np
                      do j=1,np
                         dp = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
                              ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps(i,j,tl%n0)
                         elem(ie)%state%pswet(i,j)=elem(ie)%state%pswet(i,j)+(1.0D0+elem(ie)%state%Q(i,j,k,1))*dp
                      enddo
                   enddo
                enddo
             enddo
             ...
             !                call prim_set_dry_ps(hvcoord%hyai(:),hvcoord%hybi(:),hvcoord%ps0,elem(ie)%state%pswet(:,:),&
             !                     elem(ie)%state%Q(:,:,:,1:qsize),ltrac,&
             !                     nlev,np,qsize,qsize_condensate_loading,elem(ie)%state%ps(:,:,tl%n0),elem(ie)%state%v(:,:,:,:,tl%n0),&
             !                     elem(ie)%state%T(:,:,:,tl%n0))
             !                do t=1,timelevels
             !                   elem(ie)%state%ps(:,:,t) = elem(ie)%state%ps(:,:,tl%n0)
             !                   elem(ie)%state%v(:,:,:,:,t) = elem(ie)%state%v(:,:,:,:,tl%n0)
             !                   elem(ie)%state%T(:,:,:,t)   = elem(ie)%state%T(:,:,:,tl%n0)
             !                end do
             !                do j=1,np
             !                   do i=1,np
             !                      elem(ie)%state%ps(i,j,:) = elem(ie)%state%pswet(i,j)
          ...
          do ie=nets,nete
             ...
             !             if (qsize>0) elem(ie)%state%Q(:,:,:,1) = 0.0D0                 
             !             if (ntrac>0) fvm(ie)%c(:,:,:,1,:) = 0.0D0
          end do
          if (hybrid%masterthread) then
             !             write(iulog,*) 'Test is dry so setting Q=0'
          end if
       end if
       ...
       do ie=nets,nete               
          do k=1,nlev
             do i=1,np
                do j=1,np
                   elem(ie)%state%dp3d(i,j,k,tl%n0) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
                        ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps(i,j,tl%n0)
                end do
             end do
          enddo
...
! End #ifndef CAM block
    ...
    if (runtype==0 .or. runtype==2) then
       do ie=nets,nete
          elem(ie)%derived%omega(:,:,:) = 0D0
       end do
#ifndef CAM
!
! don't initialize qdp - already initialized in inidat for CAM runs
!
     ...
     if (ntrac>0) then
       ...
       ! should be optimize and combined with the above caculatio
       if (hybrid%masterthread) then
         write(iulog,*) 'Initializing dp_fvm from spectral element dp'
         write(iulog,*) 'Make fvm%area_sphere consistent with spectral-element "areas"'
       end if
       tmp = 1.0D0
       do ie=nets,nete
         !
         ! set areas for consistency with SE areas (that are O(10E-5) incorrect) - only for CSLAM-SE coupled version
         !
         fvm(ie)%area_sphere(1:nc,1:nc) = subcell_integration(tmp, np, nc, elem(ie)%metdet) !phl - will break regression test
         do k=1,nlev
           fvm(ie)%dp_ref(k) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
                ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*p0
           fvm(ie)%dp_ref_inverse(k) = 1.0D0/fvm(ie)%dp_ref(k)
           if (rsplit==0) then
              do i=1,np
                 do j=1,np
                    elem(ie)%state%dp3d(i,j,k,tl%n0)=( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + &
           ...
           !
           ! compute dp from sub_elem_flux computation? phl?
           !
           !             fvm(ie)%dp_fvm(1:nc,1:nc,k,n0_fvm)=interpolate_gll2fvm_points(elem(ie)%derived%dp(:,:,k),deriv(hybrid%ithr)) !phl-break
           
!           fvm(ie)%dp_fvm(1:nc,1:nc,k,n0_fvm)  = subcell_integration(elem(ie)%derived%dp(:,:,k), np, nc, elem(ie)%metdet)/&
!                fvm(ie)%area_sphere(1:nc,1:nc)
#ifndef CAM
           fvm(ie)%dp_fvm(1:nc,1:nc,k,n0_fvm)  = subcell_integration(elem(ie)%state%dp3d(:,:,k,tl%n0), np, nc, elem(ie)%metdet)/&
                fvm(ie)%area_sphere(1:nc,1:nc)
#endif
         enddo
#ifndef CAM
         fvm(ie)%psc(1:nc,1:nc) = sum(fvm(ie)%dp_fvm(1:nc,1:nc,:,n0_fvm),DIM=3) +  hvcoord%hyai(1)*hvcoord%ps0
#endif
       enddo
       !       call fill_halo_fvm(elem,fvm,hybrid,nets,nete,n0_fvm) !not needed? 
       if (tracer_transport_type == TRACERTRANSPORT_CONSISTENT_SE_FVM) then
         call fvm_init_flux(elem,fvm,hybrid,nets,nete,MAX(irecons_tracer,irecons_air))
       end if
...
!=======================================================================================================!
  ...
  subroutine prim_run(elem, hybrid,nets,nete, dt, tl, hvcoord, advance_name)
    ...
    integer :: region_num_threads, ithr
...
!phlxxx       call prim_energy_halftimes(elem,hvcoord,tl,1,.true.,nets,nete)
!phlxxx       call prim_diag_scalars(elem,hvcoord,tl,1,.true.,nets,nete)
    endif
...
!$OMP BARRIER
    if (integration == "semi_imp") then
       ...
       do ie=nets,nete
          elem(ie)%state%lnps(:,:,tl%np1)= LOG(elem(ie)%state%ps(:,:,tl%np1))
       enddo
    end if
    ...
    ! =================================
    if (compute_diagnostics) then
!phlxxx       call prim_energy_halftimes(elem,hvcoord,tl,2,.false.,nets,nete)
!phlxxx       call prim_diag_scalars(elem,hvcoord,tl,2,.false.,nets,nete)
    endif
    ...
    if (compute_diagnostics) then
!phlxxx       call prim_energy_halftimes(elem,hvcoord,tl,3,.false.,nets,nete)
!phlxxx       call prim_diag_scalars(elem,hvcoord,tl,3,.false.,nets,nete)
    endif
    ...
    if (compute_diagnostics) then
!phlxxx       call prim_energy_halftimes(elem,hvcoord,tl,4,.false.,nets,nete)
!phlxxx       call prim_diag_scalars(elem,hvcoord,tl,4,.false.,nets,nete)
    endif
    ...
    !          
    if (ldry_mass_vertical_coordinates) then
       call abortmp('Error: not coded for dry mass vertical coordinates')
    else
       do ie=nets,nete
...
#endif
    call ApplyCAMForcing(elem, fvm, hvcoord,tl%n0,n0_qdp, dt_remap,nets,nete,nsubstep)
#if USE_CUDA_FORTRAN
...
#if (defined _LOOP_OPENMP)
       !$omp parallel do num_threads(vert_num_threads) private(dp_np1,dp_np1_inv)
#endif
       do k=1,nlev
           !if (k == 1) then
...
!             elem(ie)%state%Q(:,:,k,q)=elem(ie)%state%Qdp(:,:,k,q,np1_qdp)*dp_np1_inv(:,:)
  ...
  subroutine prim_commtest(hybrid,nets,nete,tl)
!
#define OVERLAP
!#undef OVERLAP
    use time_mod, only : TimeLevel_t, timelevel_update
    use control_mod, only : rsplit, hypervis_subcycle, tstep_type
    use bndry_mod, only : bndry_exchangeV, bndry_exchangeS, bndry_exchangeS_start, bndry_exchangeS_finish
    use prim_advance_mod, only : edge3p1,edge3p2
    use prim_advection_mod, only : edgeAdvp1, edgeAdv, edgeAdvQminmax
    use edge_mod, only : zeroEdgeBuffer
    ...
    ! Main timestepping loop
    ! ===================================
    ! Zero out message buffers 
    call zeroEdgeBuffer(edge3p1)
    call zeroEdgeBuffer(edge3p2)
    call zeroEdgeBuffer(edgeAdvp1)
    call zeroEdgeBuffer(edgeAdvQminmax)
    call zeroEdgeBuffer(edgeAdv)

    call MPI_Barrier(hybrid%par%comm,ierr)
    ! compute_and_apply_rhs() call multiple times
    do m=1,tstep_type
       call bndry_exchangeV(hybrid,edge3p1)
    enddo

    ! advance_hypervis_dp
    do m=1,hypervis_subcycle
      call bndry_exchangeV(hybrid,edge3p2)
      call bndry_exchangeV(hybrid,edge3p2)
    enddo
    ...
    ! euler_step call #3
#ifdef OVERLAP
    call bndry_exchangeS_start(hybrid,edgeAdvQminmax)
    call bndry_exchangeV(hybrid,edgeAdv)
    call bndry_exchangeS_finish(hybrid,edgeAdvQminmax)
#else
    call bndry_exchangeS(hybrid,edgeAdvQminmax)
    call bndry_exchangeV(hybrid,edgeAdv)
#endif
    call bndry_exchangeV(hybrid,edgeAdvp1)
      ...
      if (fvm_ideal_test == IDEAL_TEST_ANALYTICAL_WINDS) then
        ...
        do k = 1, nlev
          if (fvm_test_type == IDEAL_TEST_BOOMERANG) then
            ...
            write(*,*) "code is broken IDEAL_TEST_SOLIDBODY"
            stop
          else
...
!       vybrid = hybrid_create(hybrid%par,ithr)
    ...
    if (qsize > 0) then
      ...
      call t_startf('prim_advec_tracers_remap', t_detail_low)
      if ( tracer_num_threads > 1 ) then
        !$OMP PARALLEL NUM_THREADS(tracer_num_threads), DEFAULT(SHARED), PRIVATE(hybridnew)
        hybridnew = config_thread_region(hybrid,'tracer')
        call Prim_Advec_Tracers_remap(elem, deriv,hvcoord,flt_advection,hybridnew,&
                                      dt_q,tl,nets,nete)
        !$OMP END PARALLEL
      else
      ...
      call t_stopf('prim_advec_tracers_remap', t_detail_low)
...
!
! code below only necessary for old cslam and ff-cslam
!
