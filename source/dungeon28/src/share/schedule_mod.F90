...
contains
  ...
  subroutine genEdgeSched(par,elem, PartNumber,LSchedule,MetaVertex)
	...
	status, npackpoints, MPIinteger_t
#else
   ...
   is=1
    ...
    do j=1,ncycle
       ...
       if((MetaVertex%edges(j)%HeadVertex == PartNumber) .AND. &
          ...
          call SetCycle(elem, LSchedule,LSchedule%MoveCycle(1),MetaVertex%edges(j),HME_CYCLE_ANY)
          ...
          call SetCycle(elem, LSchedule,LSchedule%SendCycle(is),MetaVertex%edges(j),HME_CYCLE_ANY)
          ...
          call SetCycle(elem, LSchedule,LSchedule%RecvCycle(ir),MetaVertex%edges(j),HME_CYCLE_ANY)
          if(Debug) write(iulog,*)'genEdgeSched: point #14',iam
          ir = ir+1
       endif
    enddo
#else
!    print *,'Number of call to SetCycle: ',ncycle
    do j=1,ncycle
       lengthP     =  MetaVertex%edges(j)%wgtP
       lengthS     =  MetaVertex%edges(j)%wgtS
       lengthP_ghost     =  MetaVertex%edges(j)%wgtP_ghost
...
!   write(*,200) iam, nsend-numIntra,nSend
!   write(*,201) iam, numIntra,nSend
  ...
  subroutine SetCycle(elem, schedule,Cycle,Edge,ctype)
    ...
    do i=1,Edge%nmembers
       ...
       if(il .gt. 0) then 
          ...
          schedule%pIndx(schedule%pPtr)%mesgid = Edge%HeadVertex
       ...
       if(il .gt. 0) then 
          ...
          schedule%gIndx(schedule%gPtr)%mesgid=Edge%TailVertex
