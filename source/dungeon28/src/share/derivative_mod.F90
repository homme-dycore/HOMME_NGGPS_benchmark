...
! ======================================
  ...
  public :: gradient_function, gradient_routine
  public :: gradient_wk_function, gradient_wk_routine
  public :: vorticity_function, vorticity_routine
  public :: divergence_function, divergence_routine
  ...
  interface divergence_function
      module procedure divergence_nonstag_function
      module procedure divergence_stag_function
  end interface

  interface divergence_routine
      module procedure divergence_nonstag_routine
      module procedure divergence_stag_routine
  end interface

  interface gradient_function
      module procedure gradient_str_nonstag_function
      module procedure gradient_str_stag_function
  end interface

  interface gradient_routine
      module procedure gradient_str_nonstag_routine
      module procedure gradient_str_stag_routine
  end interface

  interface gradient_wk_function
      module procedure gradient_wk_nonstag_function
      module procedure gradient_wk_stag_function
  end interface

  interface gradient_wk_routine
      module procedure gradient_wk_nonstag_routine
      module procedure gradient_wk_stag_routine
  end interface
...
!
  public  :: gradient_sphere_function, gradient_sphere_routine
  public  :: gradient_sphere_wk_testcov_function, gradient_sphere_wk_testcov_routine
  public  :: gradient_sphere_wk_testcontra_function,gradient_sphere_wk_testcontra_routine ! only used for debugging
  public  :: ugradv_sphere_function, ugradv_sphere_routine
  public  :: vorticity_sphere_function, vorticity_sphere_routine
  public  :: vorticity_sphere_diag_function
  public  :: divergence_sphere_function, divergence_sphere_routine
  public  :: curl_sphere_function, curl_sphere_routine
  public  :: curl_sphere_wk_testcov_function, curl_sphere_wk_testcov_routine
!  public  :: curl_sphere_wk_testcontra  ! not coded
  public  :: divergence_sphere_wk_function, divergence_sphere_wk_routine
  public  :: laplace_sphere_wk_function, laplace_sphere_wk_routine
  public  :: vlaplace_sphere_wk_function, vlaplace_sphere_wk_routine
...
!  ================================================
  ...
  function divergence_stag_function(v,deriv) result(div)
...
!DIR$ UNROLL(NP)
  ...
  end function divergence_stag_function

  subroutine divergence_stag_routine(v,deriv,div)
...
!DIR$ UNROLL(NP)
  ...
  end subroutine divergence_stag_routine
...
!  ================================================
  ...
  function divergence_nonstag_function(v,deriv) result(div)
...
!DIR$ UNROLL(NP)
  ...
  end function divergence_nonstag_function

  subroutine divergence_nonstag_routine(v,deriv,div)
...
!DIR$ UNROLL(NP)
  ...
  end subroutine divergence_nonstag_routine
...
!  ================================================
  ...
  function gradient_wk_stag_function(p,deriv) result(dp)
...
!DIR$ UNROLL(NP)
  ...
  end function gradient_wk_stag_function

  subroutine gradient_wk_stag_routine(p,deriv,dp)
...
!DIR$ UNROLL(NP)
  ...
  end subroutine gradient_wk_stag_routine
!  ================================================
!  gradient_wk_nonstag:
! 
!  Compute the weak form gradient:
...
!  ================================================
  ...
  function gradient_wk_nonstag_function(p,deriv) result(dp)
...
!DIR$ UNROLL(NP)
  ...
  end function gradient_wk_nonstag_function

  subroutine gradient_wk_nonstag_routine(p,deriv,dp)
...
!DIR$ UNROLL(NP)
  ...
  end subroutine gradient_wk_nonstag_routine
...
!  ================================================
  ...
  function gradient_str_stag_function(p,deriv) result(dp)
...
!DIR$ UNROLL(NP)
  ...
  end function gradient_str_stag_function

  subroutine gradient_str_stag_routine(p,deriv,dp)
...
!DIR$ UNROLL(NP)
  ...
  end subroutine gradient_str_stag_routine
...
!  ================================================
  ...
  function gradient_str_nonstag_function(s,deriv) result(ds)
...
!DIR$ UNROLL(NP)
  ...
  end function gradient_str_nonstag_function

  subroutine gradient_str_nonstag_routine(s,deriv,ds)
...
!DIR$ UNROLL(NP)
  ...
  end subroutine gradient_str_nonstag_routine
...
!  ================================================
  ...
  function vorticity_function(v,deriv) result(vort)
...
!DIR$ UNROLL(NP)
  ...
  end function vorticity_function


  subroutine vorticity_routine(v,deriv,vort)
...
!DIR$ UNROLL(NP)
  ...
  end subroutine vorticity_routine
...
!DIR$ ATTRIBUTES FORCEINLINE :: gradient_sphere
  function gradient_sphere_function(s,deriv,Dinv) result(ds)
!
!   input s:  scalar
...
!DIR$ UNROLL(NP)
    ...
    ds(:,:,1)=Dinv(:,:,1,1)*v1(:,:) + Dinv(:,:,2,1)*v2(:,:)
    ds(:,:,2)=Dinv(:,:,1,2)*v1(:,:) + Dinv(:,:,2,2)*v2(:,:)

    end function gradient_sphere_function

  subroutine gradient_sphere_routine(s,deriv,Dinv,ds)
!
!   input s:  scalar
!   output  ds: spherical gradient of s, lat-lon coordinates
!
...
!DIR$ UNROLL(NP)
    ...
    end subroutine gradient_sphere_routine

  function curl_sphere_wk_testcov_function(s,deriv,elem) result(ds)
!
...
!DIR$ UNROLL(NP)
    ...
    do j=1,np
       ...
       enddo
    enddo
    end function curl_sphere_wk_testcov_function

  subroutine curl_sphere_wk_testcov_routine(s,deriv,elem,ds)
!
!   integrated-by-parts gradient, w.r.t. COVARIANT test functions
!   input s:  scalar  (assumed to be s*khat)
!   output  ds: weak curl, lat/lon coordinates
...
!DIR$ UNROLL(NP)
    ...
    do j=1,np
       ...
       enddo
    enddo
    end subroutine curl_sphere_wk_testcov_routine


  function gradient_sphere_wk_testcov_function(s,deriv,elem) result(ds)
!
!   integrated-by-parts gradient, w.r.t. COVARIANT test functions
!   input s:  scalar
...
!DIR$ UNROLL(NP)
    ...
    end function gradient_sphere_wk_testcov_function

  subroutine gradient_sphere_wk_testcov_routine(s,deriv,elem,ds)
!
!   integrated-by-parts gradient, w.r.t. COVARIANT test functions
!   input s:  scalar
!   output  ds: weak gradient, lat/lon coordinates
...
!DIR$ UNROLL(NP)
    ...
    end subroutine gradient_sphere_wk_testcov_routine 


  function gradient_sphere_wk_testcontra_function(s,deriv,elem) result(ds)
!
...
!DIR$ UNROLL(NP)
    ...
    do m=1,np
       do n=1,np
          ...
          call divergence_sphere_routine(v,deriv,elem,div)
  ...
  end function gradient_sphere_wk_testcontra_function

  subroutine gradient_sphere_wk_testcontra_routine(s,deriv,elem,ds)
!
!   integrated-by-parts gradient, w.r.t. CONTRA test functions
!   input s:  scalar
!   output  ds: weak gradient, lat/lon coordinates
...
!DIR$ UNROLL(NP)
  ...
  end subroutine gradient_sphere_wk_testcontra_routine

  function ugradv_sphere_function(u,v,deriv,elem) result(ugradv)
!
...
!
    ...
    real(kind=real_kind) :: dum_cart(np,np,3), tmp(np,np,2)
...
!                                elem%vec_sphere2cart(:,:,component,2)*v(:,:,2)
    ...
    do component=1,3
       ...
       call gradient_sphere_routine(dum_cart(:,:,component),deriv,elem%Dinv,tmp)
...
!                               dum_cart(:,:,3)*elem%vec_sphere2cart(:,:,3,component)
  ...
  end function ugradv_sphere_function


  subroutine ugradv_sphere_routine(u,v,deriv,elem,ugradv)
!
!   input:  vectors u and v  (latlon coordinates)
!   output: vector  [ u dot grad ] v  (latlon coordinates)
!
...
!                               dum_cart(:,:,3)*elem%vec_sphere2cart(:,:,3,component)
  ...
  end subroutine ugradv_sphere_routine 

  function curl_sphere_function(s,deriv,elem) result(ds)
!
!   input s:  scalar  (assumed to be  s khat)
...
!DIR$ UNROLL(NP)
    ...
    end function curl_sphere_function
  
  subroutine curl_sphere_routine(s,deriv,elem,ds)
!
!   input s:  scalar  (assumed to be  s khat)
!   output  curl(s khat) vector in lat-lon coordinates
! 
...
!DIR$ UNROLL(NP)
    ...
    end subroutine curl_sphere_routine
...
!--------------------------------------------------------------------------
  ...
  function divergence_sphere_wk_function(v,deriv,elem) result(div)
!
...
!
    ...
    ! latlon- > contra 
    vtemp(:,:,1)=(elem%Dinv(:,:,1,1)*v(:,:,1) + elem%Dinv(:,:,1,2)*v(:,:,2))
    vtemp(:,:,2)=(elem%Dinv(:,:,2,1)*v(:,:,1) + elem%Dinv(:,:,2,2)*v(:,:,2))
...
! debug the above formula using the N^4 slow formulation:
          psi=0
          psi(m,n)=1
          call gradient_sphere_routine(psi,deriv,elem%Dinv,ggtemp)
          ! latlon -> covarient
          do j=1,np
             do i=1,np
...
! grad(psi) dot v:
  ...
  end function divergence_sphere_wk_function

  subroutine divergence_sphere_wk_routine(v,deriv,elem,div)
!
!   input:  v = velocity in lat-lon coordinates
!   ouput:  div(v)  spherical divergence of v, integrated by parts
!
...
! grad(psi) dot v:
  ...
  end subroutine divergence_sphere_wk_routine
...
!DIR$ ATTRIBUTES FORCEINLINE :: vorticity_sphere
  function vorticity_sphere_function(v,deriv,elem) result(vort)
!
!   input:  v = velocity in lat-lon coordinates
...
!DIR$ UNROLL(NP)
  ...
  end function vorticity_sphere_function

  subroutine vorticity_sphere_routine(v,deriv,elem,vort)
!
!   input:  v = velocity in lat-lon coordinates
!   ouput:  spherical vorticity of v
!
...
!DIR$ UNROLL(NP)
  ...
  end subroutine vorticity_sphere_routine

  function vorticity_sphere_diag_function(v,deriv,elem) result(vort)
  !
  !   input:  v = velocity in lat-lon coordinates
...
!DIR$ UNROLL(NP)
  ...
  end function vorticity_sphere_diag_function

!DIR$ ATTRIBUTES FORCEINLINE :: divergence_sphere_routine
  subroutine divergence_sphere_routine(v,deriv,elem,div)
!
!   input:  v = velocity in lat-lon coordinates
...
!DIR$ UNROLL(NP)
  ...
  end subroutine divergence_sphere_routine

!DIR$ ATTRIBUTES FORCEINLINE :: divergence_sphere
  function divergence_sphere_function(v,deriv,elem) result(div)
!
!   input:  v = velocity in lat-lon coordinates
!   ouput:  div(v)  spherical divergence of v
!
...
!DIR$ UNROLL(NP)
  ...
  end function divergence_sphere_function


!DIR$ ATTRIBUTES FORCEINLINE :: laplace_sphere_wk
  function laplace_sphere_wk_function(s,deriv,elem,var_coef) result(laplace)
!
!   input:  s = scalar
...
!            
    ...
    call gradient_sphere_routine(s,deriv,elem%Dinv,grads)
    ...
    call divergence_sphere_wk_routine(grads,deriv,elem,laplace)

  end function laplace_sphere_wk_function

  subroutine laplace_sphere_wk_routine(s,deriv,elem,var_coef,laplace)
!
!   input:  s = scalar
!   ouput:  -< grad(PHI), grad(s) >   = weak divergence of grad(s)
!     note: for this form of the operator, grad(s) does not need to be made C0
!            
  ...
  end subroutine laplace_sphere_wk_routine

!DIR$ ATTRIBUTES FORCEINLINE :: vlaplace_sphere_wk 
  function vlaplace_sphere_wk_function(v,deriv,elem,var_coef,nu_ratio) result(laplace)
!
...
!
    ...
    if (hypervis_scaling/=0 .and. var_coef) then
       ...
       call vlaplace_sphere_wk_cartesian_routine(v,deriv,elem,var_coef,laplace)
    else  
       ! all other cases, use contra formulation:
       call vlaplace_sphere_wk_contra_routine(v,deriv,elem,laplace,var_coef,nu_ratio)
    endif

  end function vlaplace_sphere_wk_function


  subroutine vlaplace_sphere_wk_routine(v,deriv,elem,var_coef,nu_ratio,laplace)
!
!   input:  v = vector in lat-lon coordinates
!   ouput:  weak laplacian of v, in lat-lon coordinates
!
...
!
  ...
  end subroutine vlaplace_sphere_wk_routine

  function vlaplace_sphere_wk_cartesian_function(v,deriv,elem,var_coef) result(laplace)
!
!   input:  v = vector in lat-lon coordinates
!   ouput:  weak laplacian of v, in lat-lon coordinates
    ...
    do component=1,3
       call laplace_sphere_wk_routine(dum_cart(:,:,component),deriv,elem,var_coef=var_coef,&
                                                            laplace=dum_cart(:,:,component))
    enddo
...
#endif
  end function vlaplace_sphere_wk_cartesian_function

  subroutine vlaplace_sphere_wk_cartesian_routine(v,deriv,elem,var_coef,laplace)
!
!   input:  v = vector in lat-lon coordinates
!   ouput:  weak laplacian of v, in lat-lon coordinates
...
#define UNDAMPRRCART
    ...
    laplace(:,:,2)=laplace(:,:,2) + 2*elem%spheremp(:,:)*v(:,:,2)*(rrearth**2)
#endif
  end subroutine vlaplace_sphere_wk_cartesian_routine 

  function vlaplace_sphere_wk_contra_function(v,deriv,elem,var_coef,nu_ratio) result(laplace)
!
!   input:  v = vector in lat-lon coordinates
!   ouput:  weak laplacian of v, in lat-lon coordinates
!
...
!
    ...
    call divergence_sphere_routine(v,deriv,elem,div)
    call vorticity_sphere_routine(v,deriv,elem,vor)
    ...
    laplace = gradient_sphere_wk_testcov_function(div,deriv,elem) - &
         curl_sphere_wk_testcov_function(vor,deriv,elem)
...
#define UNDAMPRR
       ...
       enddo
    enddo
  end function vlaplace_sphere_wk_contra_function

  subroutine vlaplace_sphere_wk_contra_routine(v,deriv,elem,laplace,var_coef,nu_ratio)
!
!   input:  v = vector in lat-lon coordinates
!   ouput:  weak laplacian of v, in lat-lon coordinates
!
...
#define UNDAMPRR
       ...
       enddo
    enddo
  end subroutine vlaplace_sphere_wk_contra_routine 
...
!
  ...
  function subcell_Laplace_fluxes(u, deriv, elem, p, n) result(fluxes)
    ...
    call gradient_sphere_routine(u,deriv,elem%Dinv,g)
  ...
  subroutine limiter_optim_iter_full(ptens,sphweights,minp,maxp,dpmass,kbeg,kend)
    ...
    call t_startf('limiter_optim_iter_full',t_detail_low)
    do k = kbeg, kend
...
!           x(k1)=x(k1)+addmass/(c(k1)*weightsnum)
  ...
  enddo
  call t_stopf ('limiter_optim_iter_full',t_detail_low)
 
  end subroutine limiter_optim_iter_full

#endif 

#ifdef LIMITER_REWRITE
  subroutine limiter_optim_iter_full(ptens,sphweights,minp,maxp,dpmass,kbeg,kend)
    ! 
    !The idea here is the following: We need to find a grid field which is closest
    !to the initial field (in terms of weighted sum), but satisfies the min/max constraints.
