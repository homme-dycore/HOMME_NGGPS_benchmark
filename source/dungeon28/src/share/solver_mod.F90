...
contains
  ...
  function pcg_solver_nonstag(elem,       &
    ...
    call t_startf('pcg_solver_nonstag', t_detail_low)

    do ie=nets,nete
       ieptr=ie-nets+1
#if (defined _LOOP_OPENMP)
!$omp parallel do num_threads(vert_num_threads) private(iptr)
#endif
       do k=1,nlev
          iptr=1
          do j=1,np
    ...
    do while (congrad(cg,red,maxits,tol))
       call t_startf('timer_helm', t_detail_high)
...
#if (defined _LOOP_OPENMP)
!$omp parallel do num_threads(vert_num_threads) private(iptr,gradp1,gradp2)
#endif
          do k=1,nlev
...
!                                 npsq)
                ...
                !===========================================
                !   2*np*np*(np + np) Flops 
                !  SR = (4*np*np + 2*np*np + np*np)*Ld
                !  SUM(WS) = (6*np*np + 2*np*np + np*np
                !===========================================

#ifdef _WK_GRAD
                call gradient_wk_routine(reshape(cg%state(ieptr)%z(:,k),(/np,np/)),deriv,gradp(:,:,:,k,ie))
                gradp(:,:,:,k,ie)=gradp(:,:,:,k,ie)*rrearth
#else
                call gradient(cg%state(ieptr)%z(:,k),deriv,gradp(:,:,:,k,ie))
                gradp(:,:,:,k,ie)=gradp(:,:,:,k,ie)*rrearth
#endif
...
#endif
#if (defined _LOOP_OPENMP)
!$omp parallel do num_threads(vert_num_threads) private(gradp1,gradp2)
#endif
          ...
          do k=1,nlev
             if (.not.cg%converged(k)) then
                ...
                call divergence_routine(gradp(:,:,:,k,ie),deriv,div(:,:,k,ie))
                div(:,:,k,ie)=div(:,:,k,ie)*rrearth
...
!$OMP BARRIER
#endif
       call t_stopf('timer_helm', t_detail_high)
...
!$omp parallel do num_threads(vert_num_threads) private(iptr)
    ...
    call t_stopf('pcg_solver_nonstag', t_detail_low)
#endif
  end function pcg_solver_nonstag

  subroutine blkjac_init(elem, deriv,lambdasq,nets,nete,blkjac)
    use element_mod, only : element_t
    use derivative_mod, only : derivative_t, derivative_stag_t
    use derivative_mod, only : gradient_wk_routine
    use derivative_mod, only : divergence_routine
...
#if (defined _LOOP_OPENMP)
!$omp parallel do num_threads(vert_num_threads) private(p,gradp,gradp1,gradp2,div,iptr)
#endif
          do kk = 1, npsq     ! delta fn excitation index
             ...
             call gradient_wk_routine(reshape(p,(/np,np/)),deriv,gradp(:,:,:))
             gradp(:,:,:)=gradp(:,:,:)*rrearth
             ...
             call divergence_routine(gradp,deriv,div)
             div=div*rrearth
  ...
  subroutine helm_mat(ie,ElMatSize,ElementMatOut,Indices,op_data_ptr) bind(C,name='helm_mat')
    ...
    use dimensions_mod, only : nlev, np,npsq
    use derivative_mod, only : derivative_t, laplace_sphere_wk_routine 
    use derived_type_mod, only :derived_type
    implicit none
...
!    type (derivative_t)               :: deriv          ! non staggered derivative struct     (private)
    ...
    real (kind=real_kind) :: x(np,np), tmp(np,np)
    ...
    do ColIndex=1,np**2
      ...
      enddo
          ...
          !ElMatCol(:,:)= x(:,:) 
          call laplace_sphere_wk_routine(x,fptr%deriv,fptr%base(ie),var_coef=.false.,laplace=tmp)
          ElMatCol(:,:)= fptr%base(ie)%rspheremp(:,:)*(fptr%base(ie)%spheremp(:,:)*x(:,:) +&
                         alambda*tmp(:,:))
...
!
    ...
    do ie=nets,nete
       do k=1,nlev
          call laplace_sphere_wk_routine(sol(:,:,k,ie),deriv,elem(ie),var_coef=.false.,laplace=tmp)
          RHS(:,:,k,ie)=elem(ie)%spheremp(:,:)*sol(:,:,k,ie) + alambda*tmp(:,:)
...
!   
!  call build_rhs(nets,nete,np,nlev,rhsVector,c_ptr_helmholtzdata) 
!       call t_startf('timer_trilinosmlhelm', t_detail_max)
!  call helmholtz_solve(rhsVectorSize, solVector) 
!       call t_stopf('timer_trilinosmlhelm', t_detail_max)
    ...
    do while (congrad(cg,red,maxits,tol))
       do ie=nets,nete
          ...
          do k=1,nlev
             ...
             call laplace_sphere_wk_routine(x,deriv,elem(ie),var_coef=.false.,laplace=tmp)
...
tol=1.e-12
    ...
    do ie=nets,nete
       ...
       do k=1,nlev
          call laplace_sphere_wk_routine(sol(:,:,k,ie),deriv,elem(ie),var_coef=.false.,laplace=tmp)
          RHS(:,:,k,ie)=sol(:,:,k,ie) + alambda*tmp(:,:) / elem(ie)%spheremp(:,:)
       end do
       !
       !  Initialize CG solver:  set %r = residual from initial guess
       !  if initial guess = 0, then we take %r=RHS
    ...
    do while (congrad(cg,red,maxits,tol))
       ...
       do ie=nets,nete
          ...
          do k=1,nlev
             ...
             call laplace_sphere_wk_routine(x,deriv,elem(ie),var_coef=.false.,laplace=tmp)
             LHS(:,:,k,ie)=x(:,:) + alambda*tmp(:,:) / elem(ie)%spheremp(:,:)
    ...
    do while (congrad(cg,red,maxits,tol))
       do ie=nets,nete
          ...
          do k=1,nlev
             ...
             ! so only multiply x by mass;
             call laplace_sphere_wk_routine(x,deriv,elem(ie),var_coef=.false.,laplace=tmp(:,:))
             LHS(:,:,k,ie)=elem(ie)%spheremp(:,:)*x(:,:) + alambda*tmp(:,:)
          end do
          call edgeVpack(edge1, LHS(1,1,1,ie), nlev, 0, ie)
       end do
       call t_startf('bndry_exchangeV.edge1', t_detail_medium)
