...
module fvm_control_volume_mod
  ...
  type, public :: fvm_struct
    ...
    real (kind=real_kind) :: se_flux_dbg  (1-nhe:nc+nhe,1-nhe:nc+nhe,   4,nlev     ) !dbgxxx
    ...
    !
    !  x-coordinate of vertex 1: flux_vertex_cart(i,j,1,1) = fvm%acartx(i)
    !  y-coordinate of vertex 1: flux_vertex_cart(i,j,2,1) = fvm%acarty(j)
    !
    !  x-coordinate of vertex 2: flux_vertex_cart(i,j,1,2) = fvm%acartx(i+1)
    !  y-coordinate of vertex 2: flux_vertex_cart(i,j,2,2) = fvm%acarty(j  )
    !
    !  x-coordinate of vertex 3: flux_vertex_cart(i,j,1,3) = fvm%acartx(i+1)
    !  y-coordinate of vertex 3: flux_vertex_cart(i,j,2,3) = fvm%acarty(j+1)
    !
    !  x-coordinate of vertex 4: flux_vertex_cart(i,j,1,4) = fvm%acartx(i  )
    !  y-coordinate of vertex 4: flux_vertex_cart(i,j,2,4) = fvm%acarty(j+1)
    !
    real (kind=real_kind) :: flux_vertex_cart (1-nhe:nc+nhe,1-nhe:nc+nhe,2,4)
    !
    ! is side on panel edge?
    !
    integer               :: edge_cell(1:nc+1,1:nc+1)
    !
    ! flux_orient(1,i,j) = panel on which control volume (i,j) is located
    ! flux_orient(2,i,j) = cshift value for vertex permutation
    !
    real (kind=real_kind) :: flux_orient(2  ,1-nhe:nc+nhe,1-nhe:nc+nhe) 

    integer               :: flux_vec   (2,1-nhe:nc+nhe,1-nhe:nc+nhe,4) 
    !
    ! i,j: indicator function for non-existent cells (0 for corner halo and 1 elsewhere)
    !
    integer               :: ifct   (1-nhe:nc+nhe,1-nhe:nc+nhe) 

    ! indexing: (w1 or w2,x or y,ivertex,i,j) 
    real (kind=real_kind) :: weight_displ(1:2,1:2,1:4,1-nhe:nc+nhe,1-nhe:nc+nhe) 
    integer               :: rot_matrix(2,2,1-nhe:nc+nhe,1-nhe:nc+nhe)
    !
    ! coordinate_minmax(1,i,j) = min x gnomonic coordinate
    ! coordinate_minmax(2,i,j) = max x gnomonic coordinate
    ! coordinate_minmax(3,i,j) = min y gnomonic coordinate
    ! coordinate_minmax(4,i,j) = max y gnomonic coordinate
    !
!    real (kind=real_kind) :: coordinate_minmax(1:4,1-nhe:nc+nhe,1-nhe:nc+nhe) 
    !
    ...
    ! phl-opt (do not allocate for 3D runs)
    real (kind=real_kind) :: cstart(1:nc,1:nc,ntrac_d) !for idealized 2D tests - used in fvm_bench_mod and not in 3D code
!-----------------------------------------------------------------------------------!
...
!-----------------------------------------------------------------------------------!  
    ...
    real (kind=real_kind)    :: spherecentroid(1-nhe:nc+nhe,1-nhe:nc+nhe,MAX(irecons_tracer,irecons_air)-1)    
    !
    ! pre-compute some metric terms for efficiency
    !
...
!------------------------------------------------------------------------------------
    ...
    !
    real (kind=real_kind)    :: centroid_stretch(1-nhe:nc+nhe,1-nhe:nc+nhe,1:7)
    real (kind=real_kind)    :: vertex_recons_weights(4,1:MAX(irecons_tracer,irecons_air)-1,1-nhe:nc+nhe,1-nhe:nc+nhe)
  end type fvm_struct
...
subroutine create_ari(elem, fvm)
  ...
  jx_min1 = 0; jx_max1 = -1; jy_min1 = 0; jy_max1 = -1
  jx_min2 = 0; jx_max2 = -1; jy_min2 = 0; jy_max2 = -1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!        
 ...
 !
  fvm%flux_vertex_cart = 99.9D9 !for debugging
  fvm%flux_orient      = 99.9D9 !for debugging
  ...
  !
  fvm%flux_orient (1,1:nc,1:nc) = dble(fvm%faceno)
  fvm%flux_orient (2,:,:) = 0.0D0
  fvm%edge_cell           = 0
  
  fvm%rot_matrix(1,1,:,:) = 1; fvm%rot_matrix(1,2,:,:) = 0;
  fvm%rot_matrix(2,1,:,:) = 0; fvm%rot_matrix(2,2,:,:) = 1;
  ...
  if (fvm%cubeboundary>0) then
     ...
     ib = fvm%cubeboundary
     if (fvm%faceno==2) then
        if (ib==north.or.ib==nwest.or.ib==neast) fvm%flux_orient(2,1-nhe:nc+nhe,nc+1 :nc+nhe) = 1
        if (ib==south.or.ib==swest.or.ib==seast) fvm%flux_orient(2,1-nhe:nc+nhe,1-nhe:0     ) = 3       
     end if
     if (fvm%faceno==3) then
        if (ib==north.or.ib==nwest.or.ib==neast) fvm%flux_orient (2,1-nhe:nc+nhe,nc+1 :nc+nhe) = 2
        if (ib==south.or.ib==swest.or.ib==seast) fvm%flux_orient (2,1-nhe:nc+nhe,1-nhe:0     ) = 2
     end if
     if (fvm%faceno==4) then
        if (ib==north.or.ib==nwest.or.ib==neast) fvm%flux_orient (2,1-nhe:nc+nhe,nc+1 :nc+nhe) = 3
        if (ib==south.or.ib==swest.or.ib==seast) fvm%flux_orient (2,1-nhe:nc+nhe,1-nhe:0     ) = 1
     end if
     if (fvm%faceno==5) then
        if (ib==south.or.ib==swest.or.ib==seast) fvm%flux_orient (2,1-nhe:nc+nhe,1-nhe:0     ) = 2
        if (ib== west.or.ib==swest.or.ib==nwest) fvm%flux_orient (2,1-nhe:0    ,1-nhe:nc+nhe) = 3
        if (ib== east.or.ib==seast.or.ib==neast) fvm%flux_orient (2,nc+1:nc+nhe,1-nhe:nc+nhe) = 1
     end if
     
     if (fvm%faceno==6) then
        if (ib==north.or.ib==nwest.or.ib==neast ) fvm%flux_orient (2,1-nhe:nc+nhe,nc+1 :nc+nhe) = 2
        if (ib==west .or.ib==swest.or.ib==nwest ) fvm%flux_orient (2,1-nhe:0    ,1-nhe:nc+nhe)  = 1
        if (ib==east .or.ib==seast.or.ib==neast ) fvm%flux_orient (2,nc+1:nc+nhe,1-nhe:nc+nhe)  = 3
     end if
     !
     ! non-existent cells in physical space
     !
     if (fvm%cubeboundary==nwest) then
        fvm%flux_orient(2,1-nhe:0     ,nc+1 :nc+nhe) = 0
        fvm%ifct       (  1-nhe:0     ,nc+1 :nc+nhe) = 0
     else if (fvm%cubeboundary==swest) then
        fvm%flux_orient (2,1-nhe:0     ,1-nhe:0     ) = 0
        fvm%ifct        (  1-nhe:0     ,1-nhe:0     ) = 0
     else if (fvm%cubeboundary==neast) then
        fvm%flux_orient (2,nc+1 :nc+nhe,nc+1 :nc+nhe) = 0
        fvm%ifct        (  nc+1 :nc+nhe,nc+1 :nc+nhe) = 0
     else if (fvm%cubeboundary==seast) then
        fvm%flux_orient (2,nc+1 :nc+nhe,1-nhe:0     ) = 0
        fvm%ifct        (  nc+1 :nc+nhe,1-nhe:0     ) = 0
     end if
     !
     ! indicator function for edge cells - used for bowtie cases so that flow across edges/corners is not
     ! classified as bowtie
     !
     if (ib==north.or.ib==nwest.or.ib==neast ) fvm%edge_cell(1:nc+1,  nc+1)  = 1 !dbg NOT USED
     if (ib==west .or.ib==swest.or.ib==nwest ) fvm%edge_cell(     1,1:nc+1)  = 1 !dbg NOT USED
     if (ib==east .or.ib==seast.or.ib==neast ) fvm%edge_cell(  nc+1,1:nc+1)  = 1 !dbg NOT USED
     if (ib==south.or.ib==seast.or.ib==swest ) fvm%edge_cell(1:nc+1,1     )  = 1 !dbg NOT USED
     ...
     do j=1-nhe,nc+nhe
        do i=1-nhe,nc+nhe
           do ishft=1,4-nint(fvm%flux_orient(2,i,j))
              fvm%rot_matrix(:,:,i,j) = MATMUL(rot90_matrix,fvm%rot_matrix(:,:,i,j))
           end do
        enddo
     enddo
     ...
     !
     ! set indicator function for side on panel edge
     !
     !    if (ib==north) 1-nhe:nc+nhe,nc+1 :nc+nhe
     !    if (ib==nwest)
     !    if (ib==neast)
     
     !    if (ib==north) then
     !       !       if (fvm%faceno==1) then ! no rotation needed
     !       if (fvm%faceno==2) then
     !          !
     !          ! rotate 90 degrees clockwise
     !          !
     !          fvm%rot_matrix(1,1) = 0; fvm%rot_matrix(1,2) =-1;
     !          fvm%rot_matrix(2,1) = 1; fvm%rot_matrix(2,2) = 0;
     !       end if
     !       if (fvm%faceno==3) then !top panel
     !          !
     ...
     !          !
     !          fvm%rot_matrix(1,1) = 0; fvm%rot_matrix(2,1) = -1;
     !          fvm%rot_matrix(1,2) = 1; fvm%rot_matrix(2,2) =  0;
     !       end if
     !!       if (fvm%faceno==5) then !no rotation
     !       if (fvm%faceno==6) then !top panel
     !          !
     !          ! rotate 180 degrees clockwise
     !          !
     !          fvm%rot_matrix(1,1) =-1; fvm%rot_matrix(1,2) = 0;
  ...
  do j=1,nc
     do i=1,nc
        fvm%flux_vertex_cart(i,j,1,1) = fvm%acartx(i  )
        fvm%flux_vertex_cart(i,j,2,1) = fvm%acarty(j  )
        
        fvm%flux_vertex_cart(i,j,1,2) = fvm%acartx(i+1)
        fvm%flux_vertex_cart(i,j,2,2) = fvm%acarty(j  )
        
        fvm%flux_vertex_cart(i,j,1,3) = fvm%acartx(i+1)
        fvm%flux_vertex_cart(i,j,2,3) = fvm%acarty(j+1)
        
        fvm%flux_vertex_cart(i,j,1,4) = fvm%acartx(i  )
        fvm%flux_vertex_cart(i,j,2,4) = fvm%acarty(j+1)
     end do
  end do
