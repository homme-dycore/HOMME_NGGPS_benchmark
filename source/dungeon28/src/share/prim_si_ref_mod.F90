...
module prim_si_ref_mod
  ...
  public  :: prim_set_mass, prim_set_dry_ps
contains
...
9920 format(/,      13i9)
  ...
  subroutine prim_set_dry_ps(hyai,hybi,p0,pswet,tracers,ltrac,nlev,np,qsize,ncondensate_loading,psdry,uv,T)
    use kinds, only : real_kind
    use vertremap_mod, only : remap_Q_ppm
    integer                                                         , intent(in)    :: nlev, np, ncondensate_loading, qsize
    real (kind=real_kind), dimension(nlev+1 )                       , intent(in)    :: hyai, hybi
    real (kind=real_kind), dimension(np,np     )                    , intent(in )   :: pswet
    real (kind=real_kind), dimension(np,np,nlev,qsize)              , intent(inout) :: tracers
    logical              , dimension(qsize)                         , intent(in)    :: ltrac !tracer is wet
    real (kind=real_kind)                                           , intent(in)    :: p0
    real (kind=real_kind), dimension(np,np     )                    , intent(out)   :: psdry
    real (kind=real_kind), dimension(np,np,nlev)                    , intent(inout) :: T
    real (kind=real_kind), dimension(np,np,2,nlev)                  , intent(inout) :: uv

    !
    ! dry mixing ratios of water variables includeded in ps
    !


    real (kind=real_kind), dimension(np,np,nlev)  :: dp3d_wet, dp3d_dry_star, dp3d_dry    
    real (kind=real_kind), dimension(np,np,nlev) :: mtot   
    integer :: i,j,k,q



    mtot=0.0D0
    do q=1,ncondensate_loading
       do k=1,nlev
          do j=1,np
             do i=1,np
                mtot(i,j,k) = mtot(i,j,k)+tracers(i,j,k,q)
             end do
          end do
       end do
    end do
    mtot = 1/(1-mtot)
    !
    ! convert specific specific mixing ratios to dry mixing ratios
    !
    do q=1,qsize
       if (ltrac(q)) then
          do k=1,nlev
             do j=1,np
                do i=1,np
                   tracers(i,j,k,q) = mtot(i,j,k)*tracers(i,j,k,q)
                end do
             end do
          end do
       end if
    ...
    !
    ! prepare for pressure level thickness conversion
    !
    mtot=0.0D0
    do q=1,ncondensate_loading
       do k=1,nlev
   ...
   psdry = hyai(1)*p0
    do k=1,nlev
       do j=1,np
          do i=1,np
             dp3d_dry_star(i,j,k)=mtot(i,j,k)*dp3d_wet(i,j,k)
             psdry(i,j) = psdry(i,j)+dp3d_dry_star(i,j,k)
          enddo
       enddo
    end do
    do k=1,nlev
       dp3d_dry(:,:,k)=&
            (hyai(k+1) - hyai(k) )*p0 + &
            (hybi(k+1) - hybi(k) )*psdry(:,:)
    end do

    do q=1,qsize
       if (ltrac(q)) then
          tracers(:,:,:,q)=tracers(:,:,:,q)*dp3d_dry_star(:,:,:)
          call remap_Q_ppm(tracers(:,:,:,q),np,1,1,1,dp3d_dry_star,dp3d_dry)
          tracers(:,:,:,q)=tracers(:,:,:,q)/dp3d_dry(:,:,:)
       end if
    end do

    do q=1,2
       uv(:,:,q,:)=uv(:,:,q,:)*dp3d_dry_star(:,:,:)
    end do
    call remap_Q_ppm(uv(:,:,1,:),np,1,1,1,dp3d_dry_star,dp3d_dry)
    call remap_Q_ppm(uv(:,:,2,:),np,1,1,1,dp3d_dry_star,dp3d_dry)
    do q=1,2
       uv(:,:,q,:)=uv(:,:,q,:)/dp3d_dry_star(:,:,:)
    end do
    T(:,:,:)=T(:,:,:)*dp3d_dry_star(:,:,:)
    call remap_Q_ppm(T(:,:,:),np,1,1,1,dp3d_dry_star,dp3d_dry)
    T(:,:,:)=T(:,:,:)/dp3d_dry_star(:,:,:)
!
!     
  end subroutine prim_set_dry_ps
