...
subroutine remap1(Qdp,nx,qstart,qstop,qsize,dp1,dp2,hybrid)
  ...
  if (vert_remap_q_alg == 1 .or. vert_remap_q_alg == 2) then
    call t_startf('remap_Q_ppm', t_detail_low)
...
!$OMP END PARALLEL
    ...
    call t_stopf('remap_Q_ppm', t_detail_low)
    return
  endif
...
subroutine remap1_nofilter(Qdp,nx,qsize,dp1,dp2)
  ...
  integer(kind=int_kind) :: zkr(nlev+1),filter_code(nlev),peaks,im1,im2,im3,ip1,ip2, &
                            lt1,lt2,lt3,t0,t1,t2,t3,t4,tm,tp,ie,i,ilev,j,jk,k,q
  logical :: abort=.false.
  call t_startf('remap1_nofilter', t_detail_high)
...
!$omp    private(peaks_max,ip2,level1,level2,level3,level4,level5,lt1,lt2,lt3,zv1,zv2)
  ...
  call t_stopf('remap1_nofilter', t_detail_high)
  ...
  do j = 1 , nx
    do i = 1 , nx
      ...
      do q = qstart, qstop
        ...
        call compute_ppm( ao , ppmdx, coefs(:,:) )
...
subroutine compute_ppm( a , dx, coefs)
  use control_mod, only: vert_remap_q_alg
  implicit none
...
!
  ...
  use perf_utils, only         : t_detail_low, t_detail_medium, t_detail_high ! EXTERNAL
  ...
  type (EdgeBuffer_t), public :: edgeAdv, edgeAdvp1, edgeAdvQminmax, edgeAdv1,  edgeveloc
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine Prim_Advec_Tracers_spelt(elem, spelt, deriv,hvcoord,hybrid,&
    ...
    call t_startf('prim_advec_tracers_spelt', t_detail_high)
    np1 = tl%np1
    ...
    if (rsplit==0) then
       ...
       enddo
       call t_startf('bndry_exchangeV.edgeAdv1', t_detail_medium)
       call bndry_exchangeV(hybrid,edgeAdv1,location='prim_advection_mod:1029')
       call t_stopf('bndry_exchangeV.edgeAdv1', t_detail_medium)
       do ie=nets,nete
...
!       call spelt_runlimit(elem,spelt,hybrid,deriv,dt,tl,nets,nete)
    call t_stopf('prim_advec_tracers_spelt', t_detail_high)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine Prim_Advec_Tracers_fvm(elem, fvm, deriv,hvcoord,hybrid,&
    ...
    call t_startf('prim_advec_tracers_fvm', t_detail_high)
    np1 = tl%np1
...
!------------------------------------------------------------------------------------
    ...
    if (tracer_transport_type == TRACERTRANSPORT_CONSISTENT_SE_FVM) then
       ...
       call t_startf('fvm_depalg', t_detail_high)       
!       call fvm_mcgregordss(elem,fvm,nets,nete, hybrid, deriv, dt*fvm_supercycling, 3)
       ...
       call t_stopf('fvm_depalg', t_detail_high)
...
!       call fvm_mcgregordss(elem,fvm,nets,nete, hybrid, deriv, dt*fvm_supercycling, 3)
    ...
    call t_stopf('prim_advec_tracers_fvm', t_detail_high)
...
subroutine  Prim_Advec_Tracers_remap_ALE( elem , deriv , hybrid , dt , tl , nets , nete )
  ...
  call t_startf('Prim_Advec_Tracers_remap_ALE', t_detail_high)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !
  call t_startf('Prim_Advec_Tracers_remap_ALE_ghost_exchange', t_detail_high)
  do ie=nets,nete
     kptr=0
     do k=1,nlev
     do q=1,qsize
...
!         call minmax_tracers          (para_coords(i,j), neigh_q(:,:,:,elem_indexes(i,j)),f,g)
  ...
  call t_stopf('Prim_Advec_Tracers_remap_ALE_ghost_exchange', t_detail_high)
  ...
  call t_startf('Prim_Advec_Tracers_remap_ALE_Cobra', t_detail_high)
  ...
  call t_stopf('Prim_Advec_Tracers_remap_ALE_Cobra', t_detail_high)
...
!    enddo
! enddo
  call t_stopf('Prim_Advec_Tracers_remap_ALE', t_detail_high)
...
!    !------------------------------------------------------------------------------------
  ...
  do ie=nets,nete
     ...
     ! vstar   = U(x,t+1)
    do k=1,nlev
      call ugradv_sphere_routine(elem(ie)%state%v(:,:,:,k,np1), elem(ie)%derived%vstar(:,:,:,k),&
                                                                      deriv,elem(ie),vtmp(:,:,:))
...
!!$  call t_startf('Prim_Advec_Tracers_remap_ALE_parametric_coords_cart', t_detail_high)
!!$  do i=1,np
!!$    do j=1,np
!!$      n = elem_indexes(i,j)
...
!!$    end do
!!$  end do
!!$  call t_stopf('Prim_Advec_Tracers_remap_ALE_parametric_coords_cart', t_detail_high)
!  call t_startf('Prim_Advec_Tracers_remap_ALE_parametric_coords_dmap', t_detail_high)
  do i=1,np
    do j=1,np
      n = elem_indexes(i,j)
...
!  call t_stopf('Prim_Advec_Tracers_remap_ALE_parametric_coords_dmap', t_detail_high)
! do i=1,np
!   do j=1,np
...
!    call t_barrierf('sync_prim_advec_tracers_remap_k2', hybrid%par%comm)
    call t_startf('prim_advec_tracers_remap_rk2', t_detail_low)
!    call extrae_user_function(1)
    ...
    do ie=nets,nete 
      do k=kbeg,kend
        ...
        call divergence_sphere_routine(gradQ,deriv,elem(ie),elem(ie)%derived%divdp(:,:,k))
...
!    call extrae_user_function(0)
    ...
    call t_stopf('prim_advec_tracers_remap_rk2', t_detail_low)
...
!-----------------------------------------------------------------------------
  ...
  call t_startf('euler_step', t_detail_low)
  ...
  if ( limiter_option == 8  ) then
    ...
    if ( rhs_multiplier == 0 ) then
      ...
      call t_startf('euler_neighbor_minmax1', t_detail_high)
      call neighbor_minmax(hybrid,edgeAdvQminmax,nets,nete,qmin(:,:,nets:nete),qmax(:,:,nets:nete))
      call t_stopf('euler_neighbor_minmax1', t_detail_high)
    endif
...
#else
      call t_startf('euler_neighbor_minmax2', t_detail_high)
      call neighbor_minmax(hybrid,edgeAdvQminmax,nets,nete,qmin(:,:,nets:nete),qmax(:,:,nets:nete))
      call t_stopf('euler_neighbor_minmax2', t_detail_high)
      call biharmonic_wk_scalar(elem,qtens_biharmonic,deriv,edgeAdv,hybrid,nets,nete) 
...
!           elem(ie)%derived%psdiss_biharmonic(i,j)
    ...
    do q = qbeg, qend
      do k = kbeg, kend
        ...
        call divergence_sphere_routine( gradQ , deriv , elem(ie), div )
...
!JMD !$OMP BARRIER
  ...
  call t_startf('bndry_exchangeV.edgeAdvp1', t_detail_medium)
  call bndry_exchangeV( hybrid , edgeAdvp1,location='prim_advection_mod:2603')
  call t_stopf('bndry_exchangeV.edgeAdvp1', t_detail_medium)
  ...
  do ie = nets, nete 
    ...
    enddo
  enddo
  call t_stopf('euler_step', t_detail_low)
...
!-----------------------------------------------------------------------------
  ...
  use element_mod, only : element_t
  use derivative_mod, only : derivative_t, edge_flux_u_cg, gll_to_dgmodal, dgmodal_to_gll
  use derivative_mod, only : divergence_sphere_wk_routine
  ...
  ! local
  real (kind=real_kind), dimension(np,np)    :: divdp, htemp
...
! nelemd
  ...
  call t_startf('euler_step_dg', t_detail_low)
  ...
  if(DSSopt==DSSno_var)then
     call t_startf('bndry_exchangeV.edgeAdv', t_detail_medium)
     call bndry_exchangeV( hybrid, edgeAdv,location='prim_advection_mod:2857')
     call t_stopf('bndry_exchangeV.edgeAdv', t_detail_medium)
  else
     call t_startf('bndry_exchangeV.edgeAdvp1', t_detail_medium)
     call bndry_exchangeV( hybrid, edgeAdvp1,location='prim_advection_mod:2861')
     call t_stopf('bndry_exchangeV.edgeAdvp1', t_detail_medium)
  endif
  ...
  do ie=nets,nete
     ...
     do q=qbeg,qend
        do k=kbeg,kend
           ...
           call divergence_sphere_wk_routine(vtemp,deriv,elem(ie),htemp)
           divdp = htemp + &
...
!JMD !$OMP BARRIER
  ...
  call t_stopf('euler_step_dg', t_detail_low)
...
!  real (kind=real_kind), dimension(:,:), pointer :: spheremp,rspheremp
  ...
  call t_startf('advance_hypervis_scalar', t_detail_low)
...
!JMD!$OMP BARRIER
  ...
  enddo
  call t_stopf('advance_hypervis_scalar', t_detail_low)
