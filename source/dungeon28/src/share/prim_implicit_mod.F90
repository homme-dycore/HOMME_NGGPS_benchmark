...
!    if (hybrid%masterthread) print*, "integration method, gamma",method,gam

    !JMD  call t_barrierf('sync_residual', hybrid%par%comm)
    call t_startf('residual', t_detail_high)

    call t_startf('residual_int', t_detail_high)
    !fvtens = 0.0d0
    !fttens = 0.0d0
    !fpstens = 0.0d0
    !fdptens = 0.0d0
    ...
    do ie=nets,nete
       do j=1,np
          do i=1,np
             fptr%base(ie)%state%ps(i,j,np1) = xstate(lx)
             lx = lx+1
          end do
       end do
    end do
    call t_stopf('residual_int', t_detail_high)

    call t_startf('residual_cal', t_detail_high)

    do ie=nets,nete
       !     ps => fptr%base(ie)%state%ps(:,:,n0)
       ...
       !
       ! pressure here should probably be full pressure!
       !
       ! ==================================================
       ! vertically eulerian only needs grad(ps)
       if (rsplit==0) &
            call gradient_sphere_routine(fptr%base(ie)%state%ps(:,:,n0),deriv,fptr%base(ie)%Dinv,grad_ps)

       ! ============================
       ! compute p and delta p
       ! ============================
       !       ph(:,:,k)   = hvcoord%hyai(k)*hvcoord%ps0 + hvcoord%hybi(k)*fptr%base(ie)%state%ps(:,:,n0)

#if (defined _LOOP_OPENMP)
       !$omp parallel do num_threads(vert_num_threads) private(v1,v2,vtemp)
#endif
       do k=1,nlev
          if (rsplit==0) then
             dp(:,:,k) = (hvcoord%hyai(k+1)*hvcoord%ps0 + hvcoord%hybi(k+1)*fptr%base(ie)%state%ps(:,:,n0)) &
                  - (hvcoord%hyai(k)*hvcoord%ps0 + hvcoord%hybi(k)*fptr%base(ie)%state%ps(:,:,n0))
             p(:,:,k)   = hvcoord%hyam(k)*hvcoord%ps0 + hvcoord%hybm(k)*fptr%base(ie)%state%ps(:,:,n0)
             ...
             call gradient_sphere_routine(p(:,:,k),deriv,fptr%base(ie)%Dinv,grad_p(:,:,:,k))
          endif
          ...
          !
          ! =========================================
          call divergence_sphere_routine(vtemp,deriv,fptr%base(ie),divdp(:,:,k)) 
          call vorticity_sphere_routine(fptr%base(ie)%state%v(:,:,:,k,n0),deriv,fptr%base(ie),vort(:,:,k))
...
! unlike the RK/LF code, do not average over subcycled dynamics to get these values
       do k=1,nlev  
          ...
          !
          write(*,*) " WARNING: omega_p changed to omega (need to divide by p)"
          stop
          fptr%base(ie)%derived%omega(:,:,k) = omega_p(:,:,k)
       enddo
...
#if (defined _LOOP_OPENMP)
       !$omp parallel do num_threads(vert_num_threads)  &
       !$omp    private(v1,v2,E,Ephi,vtemp,vgrad_T,gpterm,glnps1,glnps2)
#endif
       do k=1,nlev
          do j=1,np
             do i=1,np
          ...
          ! ================================================
          call gradient_sphere_routine(fptr%base(ie)%state%T(:,:,k,n0),deriv,fptr%base(ie)%Dinv,vtemp(:,:,:))
          do j=1,np
             do i=1,np
                v1     = fptr%base(ie)%state%v(i,j,1,k,n0)
                v2     = fptr%base(ie)%state%v(i,j,2,k,n0)
          ...
          ! vtemp = grad ( E + PHI )
          call gradient_sphere_routine(Ephi(:,:),deriv,fptr%base(ie)%Dinv,vtemp)
       ...
       fpstens(:,:,ie) = fptr%base(ie)%spheremp(:,:)* &
             (1+gam)*( (fptr%base(ie)%state%ps(:,:,np1) - fptr%base(ie)%state%ps(:,:,nm1) )*dti - &
            gam*(fptr%base(ie)%state%ps(:,:,nm1) - fptr%base(ie)%state%ps(:,:,nm))*dti + &
            sdot_sum(:,:) )
    ...
    call bndry_exchangeV(hybrid,edge3p1,location='residual')
    ...
    call t_stopf('residual_cal', t_detail_high)
    call t_startf('residual_fin', t_detail_high)
    lx = 1
    do ie=nets,nete
       do k=1,nlev
          do j=1,np
    ...
    call t_stopf('residual_fin', t_detail_high)
    ...
    !$OMP BARRIER
#endif
    call t_stopf('residual', t_detail_high)
  ...
  subroutine update_prec_state(xs, nelemd, c_ptr_to_object) bind(C,name='update_prec_state')
    ...
    !          do j=1,np
    !            do i=1,np
    !              if (n==4) fptr%base(ie)%state%ps(i,j,np1)  = xs(lx)
    !              lx = lx+1
    !            end do  !np
    !          end do  !np
    !        end if ! nvar
  ...
  subroutine test_id(xs, nelemd, fx, c_ptr_to_object) bind(C,name='test_id')
    ...
    !    call t_startf('test id', t_detail_max)
    !call flush(6)
    !write(6,*)'startf'
    !call flush(6)
    ...
    !    call t_stopf('test id', t_detail_max)
  ...
  subroutine rhs_advance_hypervis(edge3,fv,ft,fp,elem,hvcoord,hybrid,deriv,nt,nets,nete,dt2)
    ...
    call t_startf('rhs_advance_hypervis', t_detail_low)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (hypervis_order == 2) then
       do ic=1,hypervis_subcycle
          ...
          do ie=nets,nete
             ...
             if (nu_p>0) then
#if 0
                elem(ie)%derived%psdiss_ave(:,:)= elem(ie)%state%ps(:,:,nt)
          ...
          call bndry_exchangeV(hybrid,edge3,location='rhs_advance_hypervis')
    ...
    call t_stopf('rhs_advance_hypervis', t_detail_low)
  ...
  subroutine rhs_advance_hypervis_dp(edge3,fv,ft,fp3,elem,hvcoord,hybrid,deriv,nt,nets,nete,dt2)
    ...
    if (nu_s == 0 .and. nu == 0 .and. nu_p==0 ) return;
    !JMD  call t_barrierf('sync_advance_hypervis', hybrid%par%comm)
    call t_startf('rhs_advance_hypervis_dp', t_detail_low)

! call get_loop_ranges(hybrid,kbeg=kbeg,kend=kend)
! KJE need to add kblk code here for this code to run going forward
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    if (hypervis_order == 2) then
       do ic=1,hypervis_subcycle
...
#if (defined _LOOP_OPENMP)
             !$omp parallel do num_threads(vert_num_threads) 
             !$omp private(lap_t,lap_dp,lap_v,nu_scale_top,dpdn,dpdn0,utens_tmp,vtens_tmp,ttens_tmp,dptens_tmp)
#endif
             do k=1,nlev
                ! advance in time.
                ! note: DSS commutes with time stepping, so we can time advance and then DSS.
          ...
          call t_startf('bndry_exchangeV.edge3', t_detail_medium)
          call bndry_exchangeV(hybrid,edge3,location='rhs_advance_hypervis_dp')
          call t_stopf('bndry_exchangeV.edge3', t_detail_medium)
    ...
    call t_stopf('rhs_advance_hypervis_dp', t_detail_low)
