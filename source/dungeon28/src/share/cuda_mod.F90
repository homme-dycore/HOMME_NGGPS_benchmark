...
!
!As a general rule, all of the routines in here will be called within a threaded context (assuming LOOP_OPENMP is not
...
contains
  ...
  subroutine cuda_mod_init(elem,hybrid,deriv,hvcoord)
    ...
    write(*,*) 'ERROR: Do not use _LOOP_OPENMP and CUDA FORTRAN'
    stop
#endif
! NP > 4 is not supported due to shared memory constraints
#if NP > 4
...
!    elem(ie)%derived%eta_dot_dpdn(:,:,:)=0
    if (rsplit==0) then
      ...
      do k=1,nlev
        dp_h(:,:,k,ie) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps(:,:,np1)
      ...
      ! update final ps 
      elem(ie)%state%ps(:,:,np1) = hvcoord%hyai(1)*hvcoord%ps0 + sum(elem(ie)%state%dp3d(:,:,:,np1),3)
      do k=1,nlev
        dp_h(:,:,k,ie) = ( hvcoord%hyai(k+1) - hvcoord%hyai(k) )*hvcoord%ps0 + ( hvcoord%hybi(k+1) - hvcoord%hybi(k) )*elem(ie)%state%ps(:,:,np1)
