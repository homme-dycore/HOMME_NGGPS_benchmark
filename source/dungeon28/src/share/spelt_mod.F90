...
subroutine spelt_runtest(elem,spelt,hybrid,deriv,tstep,tl,nets,nete)
  ...
  call t_startf('SPELT scheme', t_detail_max)
...
!-----------------------------------------------------------------------------------!
  call t_startf('SPELT Communication', t_detail_max)
  call ghost_exchangeV(hybrid,cellghostbuf,nipm,nep,ntrac)
  call t_stopf('SPELT Communication', t_detail_max)
!-----------------------------------------------------------------------------------!
  call t_startf('SPELT Unpacking', t_detail_max)
  do ie=nets,nete
    call ghostVunpack2d(cellghostbuf,spelt(ie)%c,nipm, nep,nlev,ntrac,0, tl%np1, timelevels,elem(ie)%desc)
  end do
  call t_stopf('SPELT Unpacking', t_detail_max)
  call t_stopf('SPELT scheme', t_detail_max)
...
!!!! only needed for filter
  ...
  call t_startf('SPELT Communication1', t_detail_max)
  call ghost_exchangeV(hybrid,factorR,nhe,nc,ntrac)
  call t_stopf('SPELT Communication1', t_detail_max)
...
!-----------------------------------------------------------------------------------!
  call t_startf('SPELT Communication2', t_detail_max)
  call ghost_exchangeV(hybrid,cellghostbuf,nipm,nep,ntrac)
  call t_stopf('SPELT Communication2', t_detail_max)
!-----------------------------------------------------------------------------------!
  call t_startf('SPELT Unpacking2', t_detail_max)
  do ie=nets,nete
    call ghostVunpack2d(cellghostbuf,spelt(ie)%c,nipm, nep,nlev,ntrac,0, tl%np1, timelevels,elem(ie)%desc)
  end do
  call t_stopf('SPELT Unpacking2', t_detail_max)
  call t_stopf('SPELT scheme', t_detail_max)
...
subroutine spelt_init2(elem,spelt,hybrid,nets,nete,tl)
  ...
  use derivative_mod, only : derivative_t, derivinit, v2pinit, interpolate_gll2spelt_points
...
subroutine spelt_mcgregordss(elem,spelt,nets,nete, hybrid, deriv, tstep, ordertaylor)
  ...
  do  order=1,ordertaylor
    ...
    call bndry_exchangeV(hybrid,edgeveloc,location='spelt_mcgregordss')
...
!         ugradvtmp(:,:,:)=ugradv_sphere(fvm(ie)%vn0(:,:,:,k),elem(ie)%derived%vstar(:,:,:,k),deriv,elem(ie))
        call ugradv_sphere_routine(elem(ie)%derived%vstar(:,:,:,k),spelt(ie)%vn0(:,:,:,k),deriv,elem(ie),ugradvtmp(:,:,:))
    ...
    enddo
    call t_startf('bndry_exchangeV.edgeveloc', t_detail_medium)
    call bndry_exchangeV(hybrid,edgeveloc,location='spelt_rkdss')
    call t_stopf('bndry_exchangeV.edgeveloc', t_detail_medium)
    do ie=nets,nete
