...
module namelist_mod
  ...
  !-----------------
#ifndef CAM
  use time_mod, only : tstep, ndays,nmax, nendstep,secpday, smooth, secphr, nsplit, phys_tscale, dt_phys
#endif
  !-----------------
  use parallel_mod, only : parallel_t,  iam, abortmp, &
       partitionfornodes, useframes, mpireal_t, mpilogical_t, mpiinteger_t, mpichar_t, &
       boundaryCommMethod, HME_BNDRY_P2P, HME_BNDRY_A2A, HME_BNDRY_A2AO, HME_BNDRY_GET1, HME_BNDRY_GET2, &
       HME_BNDRY_PUT1, HME_BNDRY_PUT2, HME_BNDRY_MASHM
  !-----------------
...
#ifndef CAM
  use fvm_line_integrals_mod, only: fvm_ideal_test, ideal_test_off, ideal_test_analytical_departure, ideal_test_analytical_winds
...
! ============================================
  subroutine readnl(par)
    ...
    character(len=32) :: boundaryCommMethodName = 'AUTO'
    ...
    namelist /ctl_nl/ PARTMETHOD,    &       ! Mesh partitioning method (METIS)
                     ...
                     horz_num_threads,      &       ! Number of horizontal threads per process 
                     vert_num_threads,      &       ! Number of vertical threads per process 
                     tracer_num_threads,      &       ! Number of tracer threads per process
                     boundaryCommMethodName, &    ! Edge Communication method (AUTO,A2A,A2AO,P2P,GET,PUT,MASHM,etc.)
                     limiter_option, &
                     smooth,        &        ! Timestep Filter
                     dt_phys,       &        ! physics time-step
                     omega,         &
...
!   write(iulog,*) "masterproc=",par%masterproc
    ...
    if (par%masterproc) then
      ...
      PARTMETHOD = SFCURVE
       ...
       ! Set the (internal) variable for boundary communication methods
       ! (boundaryCommMethod) based on the text supplied via the namelist
       ! variable (boundaryCommMethodName):
       boundaryCommMethodName = shr_string_toUpper(boundaryCommMethodName)     ! Convert to upper-case
       select case (boundaryCommMethodName)
         case("P2P")
           boundaryCommMethod = HME_BNDRY_P2P
           write(iulog,*) "Setting boundaryCommMethod to 'P2P' => ",HME_BNDRY_P2P
         case("A2A")
           boundaryCommMethod = HME_BNDRY_A2A
           write(iulog,*) "Setting boundaryCommMethod to 'A2A' => ",HME_BNDRY_A2A
         case("A2AO")
           boundaryCommMethod = HME_BNDRY_A2AO
           write(iulog,*) "Setting boundaryCommMethod to 'A2AO' => ",HME_BNDRY_A2AO
         case("GET")
         case("GET1")
           boundaryCommMethod = HME_BNDRY_GET1
           write(iulog,*) "Setting boundaryCommMethod to 'GET1' => ",HME_BNDRY_GET1
         case("GET2")
           boundaryCommMethod = HME_BNDRY_GET2
           write(iulog,*) "Setting boundaryCommMethod to 'GET2' => ",HME_BNDRY_GET2
         case("PUT")
         case("PUT1")
           boundaryCommMethod = HME_BNDRY_PUT1
           write(iulog,*) "Setting boundaryCommMethod to 'PUT1' => ",HME_BNDRY_PUT1
         case("PUT2")
           boundaryCommMethod = HME_BNDRY_PUT2
           write(iulog,*) "Setting boundaryCommMethod to 'PUT2' => ",HME_BNDRY_PUT2

         case("MASHM")
! Compiling with MASHM defines HAVE_MASHM (per the CMake files)
#ifdef HAVE_MASHM 
           boundaryCommMethod = HME_BNDRY_MASHM
           write(iulog,*) "Setting boundaryCommMethod to 'MASHM' => ",HME_BNDRY_MASHM
#else
           CALL abortmp('MASHM communications selected, but library not found.')
#endif
         case default  ! In the default case, we're going to auto-detect a method:
           ! For now, we're setting 'auto' to P2P - will change this later to
           ! autodetect optimal method
           boundaryCommMethod = HME_BNDRY_P2P
           write(iulog,*) "Setting boundaryCommMethod to 'P2P (AUTO)' => ",HME_BNDRY_P2P
         end select
...
! ^ ifdef  _PRIMDG
       ...
       if (test_case=="held_suarez0" .or. &
           ...
           test_case(1:15)=="umjs_baroclinic" .or. &
...
!JMD    call MPI_bcast(NTHREADS  ,1,MPIinteger_t,par%root,par%comm,ierr)
    call MPI_bcast(horz_num_threads,1,MPIinteger_t,par%root,par%comm,ierr)
    call MPI_bcast(vert_num_threads,1,MPIinteger_t,par%root,par%comm,ierr)
    call MPI_bcast(tracer_num_threads,1,MPIinteger_t,par%root,par%comm,ierr)
    call MPI_bcast(ndays     ,1,MPIinteger_t,par%root,par%comm,ierr)
    call MPI_bcast(dt_phys,1,MPIreal_t   ,par%root,par%comm,ierr)
    ...
    call MPI_bcast(commtest,1,MPIlogical_t,par%root,par%comm,ierr)
    ...
    call MPI_bcast(boundaryCommMethod, 1, MPIinteger_t, par%root, par%comm, ierr)
...
!phl         call abortmp('user specified qsize should only be > 0 when tracer_transport_type is se_gll')
!phl      end if
    else if (trim(tracer_transport_method) == 'consistent_se_cslam_fvm') then
...
!phl      end if
      ...
      call abortmp('namelist_mod: Unknown tracer transport method: '//trim(tracer_transport_method))
    end if
...
!=======================================================================================================!
    ...
    if (par%masterproc) then
       ...
       write(iulog,*)"readnl: dt_phys       = ",dt_phys
       ...
       write(iulog,*)"readnl: horz_num_threads = ",horz_num_threads
       write(iulog,*)"readnl: vert_num_threads = ",vert_num_threads
       write(iulog,*)"readnl: tracer_num_threads = ",tracer_num_threads
       write(iulog,*)"readnl: boundaryCommMethodName = ", boundaryCommMethodName
       ...
       write(iulog,*)"readnl: restartfreq   = ",restartfreq
