...
!    call t_adj_detailf(+3)
     call t_startf('initedgebuffer', t_detail_high)
...
!$OMP MASTER
    ...
    do j=2,SIZE(pSchedule%pIndx)
       ...
       if(il>0 .and. ie >0) then
          ...
          if(mesgid .ne. iam) then  ! don't enter if this is a move cycle where (mesgid == iam)
             if(mesgid .ne. dst0) then 
                icycle=icycle+1
                edge%sdisplsFull(icycle) = edge%putmap(il,ie)
                dst0=mesgid
             endif
             edge%scountsFull(icycle) = edge%scountsFull(icycle)+len
          endif
          ilm1=il
          iem1=ie
          lenm1=len
       ...
       if(il>0 .and. ie >0) then 
          ...
          if(mesgid .ne. iam) then ! don't enter if this is a move cycle where (mesgid == iam)
             if(mesgid .ne. src0) then
                edge%rdisplsFull(icycle+1) = edge%getmap(il,ie)
                icycle=icycle+1
                src0=mesgid
             endif
             edge%rcountsFull(icycle) = edge%rcountsFull(icycle)+len
          endif
          ilm1=il
          iem1=ie 
          lenm1=len
    ...
    if(edge%bndry_type == HME_BNDRY_GET1 .or. &
       ...
       enddo
       call MPI_Waitall(nSendCycles,edge%Srequest,edge%status,ierr)
       call MPI_Waitall(nRecvCycles,edge%Rrequest,edge%status,ierr)
     endif
...
!$OMP BARRIER
     ...
     call t_stopf('initedgebuffer', t_detail_high)
!    call t_adj_detailf(-3)
