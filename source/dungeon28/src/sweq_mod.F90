...
contains
  subroutine sweq(elem,fvm,edge1,edge2,edge3,red,par,ithr,nets,nete)
...
#ifdef TRILINOS
#if SWIM_USE_OPENACC
    use implicit_mod, only : advance_imp_nonstag, acc_imp_int, acc_imp_fin
#else
    ...
    use fvm_mod, only : fvm_init2,fill_halo_fvm
...
! state_object is a derived data type passed thru noxinit as a pointer
  ...
  real (kind=real_kind)                          :: maxcflx, maxcfly  
    ...
    call t_startf('sweq', t_detail_low)
...
!    hybrid = hybrid_create(par,ithr,max_num_threads)
...
!   call test_subcell_dss_fluxes_again(elem,deriv,nets,nete)
    ...
    call t_startf('sweq_gvinit', t_detail_high)
    if (topology == "cube") then
       if (runtype .eq. 1) then 
          ...
          if (test_case(1:5) == "swtc1") then
             ...
             call fill_halo_fvm(elem,fvm,hybrid,nets,nete,tl%n0)
...
!             end if
    ...
    call t_stopf('sweq_gvinit', t_detail_high)
...
!       end if
    ...
    if(hybrid%masterthread) then 
       write(6,*) "~~~~~~~~~~~~~~~~~~"
       write(6,*) "  Initialize ACC  "
       write(6,*) "~~~~~~~~~~~~~~~~~~"
    endif 

    call acc_imp_int(elem, deriv, nets,nete)
    ...
    do while(tl%nstep<nmax)
       call t_startf('sweq_loop', t_detail_high)
 ...
 call t_startf('contra', t_detail_high)
        do ie=nets,nete
         do k=1,nlev
          do i=1,np
           do j=1,np
...
! need  to  convert  xstate  to  latlon
 ...
 call t_stopf('contra', t_detail_high)
...
!!! RDL            call tc8_invariants(90, tl, pmean, edge2, deriv, hybrid, nets, nete)
 ...
 call t_stopf('sweq_loop', t_detail_high)
    end do
...
!     end if
    ...
    ! ==========================
    call t_startf('sweq finish', t_detail_high)
#ifdef PIO_INTERP
    call interp_movie_finish
#else
    call shal_movie_finish
#endif
    call t_stopf('sweq finish', t_detail_high)
    call t_stopf('sweq', t_detail_low)
  end subroutine sweq
...
!JMD    hybrid = hybrid_create(par,ithr,NThreads,nets,nete)
!JMD    hybrid = hybrid_create(par,ithr,max_num_threads)
    hybrid = config_thread_region(par,'horizontal')
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine Shal_Advec_Tracers_fvm(elem, fvm, deriv,hybrid,&
      ...
      call t_startf('shal_advec_tracers_fvm', t_detail_low)
...
!-------END new McGregor scheme--------
      ...
      call t_stopf('shal_advec_tracers_fvm', t_detail_low)
