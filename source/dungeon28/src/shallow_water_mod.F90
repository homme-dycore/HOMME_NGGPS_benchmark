...
module shallow_water_mod
  ...
  ! ------------------------
  use derivative_mod, only : derivative_t, derivative_stag_t, vorticity_routine
  use derivative_mod, only : vorticity_sphere_routine, gradient_wk_routine
  use derivative_mod, only : divergence_routine, divergence_sphere_routine
  use derivative_mod, only : laplace_sphere_wk_routine, vlaplace_sphere_wk_routine
  ! ------------------------
  ...
  subroutine sweq_invariants(elem, iounit,tl,pmean,edge3,deriv,hybrid,nets,nete)
    ...
    if (nu > 0 .or. nu_s > 0 ) then
       ...
       if (hypervis_order==2) then
          call biharmonic_wk(elem,diss_p,diss_v,deriv,edge3,hybrid,n0,nets,nete,1,nlev)
       else
          do ie=nets,nete
             v  => elem(ie)%state%v(:,:,:,k,n0)
             E(:,:)=elem(ie)%state%p(:,:,k,n0) + elem(ie)%state%ps(:,:)
             call laplace_sphere_wk_routine (E,deriv,elem(ie),var_coef=.false.,laplace=diss_p(:,:,k,ie))
             call vlaplace_sphere_wk_routine(v,deriv,elem(ie),var_coef=.false.,laplace=diss_v(:,:,:,k,ie))
          enddo
       endif
...
!but mass conservation output will be done for all levels 
       ...
       call divergence_sphere_routine(vlatlon,deriv,elem(ie),div(:,:,ie)) ! latlon vector -> scalar 
       call vorticity_sphere_routine(vlatlon,deriv,elem(ie),vor(:,:,ie)) ! latlon vector -> scalar 
...
#ifdef _WK_GRAD
       call gradient_wk_routine(elem(ie)%state%ps,deriv,elem(ie)%state%gradps(:,:,:))
       elem(ie)%state%gradps(:,:,:)=elem(ie)%state%gradps(:,:,:)*rrearth
#else
       call gradient_routine(elem(ie)%state%ps,deriv,elem(ie)%state%gradps(:,:,:))
       elem(ie)%state%gradps(:,:,:)=elem(ie)%state%gradps(:,:,:)*rrearth
#endif
...
30  format(f11.6,4x,e13.6)
  ...
  subroutine tc5_invariants(elem, iounit,tl,pmean,edge2,deriv,hybrid,nets,nete)
    ...
    do ie=nets,nete
       ...
       call vorticity_routine(vco,deriv, zeta(:,:,ie))
       zeta(:,:,ie)= zeta(:,:,ie)*rrearth
       call divergence_routine(gv,deriv,div(:,:,ie))
       div(:,:,ie)=div(:,:,ie)*rrearth
