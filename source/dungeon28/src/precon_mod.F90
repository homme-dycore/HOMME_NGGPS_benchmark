...
contains
  ...
  function pcg_presolver_nonstag(pptr, rhs) result(x) 
    ...
    use derivative_mod, only : derivative_t, gradient_wk_routine, gradient_routine, divergence_routine
...
! declarations of unpacked pptr pointer
   ...
   do ie=nets,nete
    ...
    call t_startf('pcg_presolver', t_detail_high)
    ...
    do while (congrad(cg,red,maxits,tol))
       call t_startf('in_congrad', t_detail_high)
...
#else
                call gradient_routine(cg%state(ieptr)%z(:,k),deriv,gradp(:,:,:,k,ie))
                gradp(:,:,:,k,ie)=gradp(:,:,:,k,ie)*rrearth
#endif
       ...
       call t_stopf('in_congrad', t_detail_high)
...
!                !       x(i,j,k,ie)=cg%wrk3(iptr,k,ieptr)
    ...
    call t_stopf('pcg_presolver', t_detail_high)
