...
! set MPI communicator
      ...
      if ( use_ddpdd_sum ) then
         ...
         call t_startf('repro_sum_ddpdd', t_detail_high)

         call repro_sum_ddpdd(arr, arr_gsum, nsummands, dsummands, &
                              nflds, mpi_comm)
         repro_sum_fast = 1

         call t_stopf('repro_sum_ddpdd', t_detail_high)

      else

         call t_startf('repro_sum_int', t_detail_high)
...
! return upper bounds on observed maxima 
         ...
         call t_stopf('repro_sum_int', t_detail_high)
...
#endif
            call t_startf('nonrepro_sum', t_detail_high)
! record statistic
            nonrepro_sum = 1
...
#endif
            call t_stopf('nonrepro_sum', t_detail_high)
