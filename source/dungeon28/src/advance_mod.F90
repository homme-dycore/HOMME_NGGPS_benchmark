...
contains
  ...
  subroutine advance_nonstag( elem, edge2,  edge3,  deriv,  flt,   hybrid,  &
    ...
    call t_startf('advance_nonstag', t_detail_high)
    ...
    if (smooth/=0) then
    call t_startf('smooth', t_detail_high)
    do ie=nets,nete
       ! ====================================================
       ! apply Robert filter
       ! ====================================================
    ...
    call t_stopf('smooth', t_detail_high)
    endif

    call t_stopf('advance_nonstag', t_detail_high)
...
!--------------------------------------------------------------------------------------------
  ...
  subroutine advance_nonstag_rk( MyRk, elem, edge2,  edge3,  deriv,  flt,   hybrid,  &
    ...
    use hybrid_mod, only : hybrid_t
    ! ---------------------
    use derivative_mod, only : derivative_t, gradient_sphere_routine, divergence_sphere_routine
    use derivative_mod, only : vorticity_sphere_routine,divergence_sphere_wk_routine
    use derivative_mod, only : edge_flux_u_cg, dgmodal_to_gll, gll_to_dgmodal
    ! ---------------------
...
!applying viscosity to q field
        ...
        do ie=nets,nete
           ...
           enddo
        enddo
        call biharmonic_wk(elem,ptens,vtens,deriv,edge3,hybrid,n0,nets,nete,1,nlev)
       ...
       do ie=nets,nete
          ...
          do k=1,nlev
             ...
             call gradient_sphere_routine(E,deriv,elem(ie)%Dinv,grade)       ! scalar -> latlon vector
             !grade = gradient_sphere_wk(E,deriv,elem(ie)%Dinv)       ! scalar -> latlon vector
             call vorticity_sphere_routine(ulatlon,deriv,elem(ie),zeta) ! latlon vector -> scalar 
             if (tracer_advection_formulation==TRACERADV_UGRADQ) then
                call gradient_sphere_routine(elem(ie)%state%p(:,:,k,n0),deriv,elem(ie)%Dinv,gradh)
                div = ulatlon(:,:,1)*gradh(:,:,1)+ulatlon(:,:,2)*gradh(:,:,2)
             else
                call divergence_sphere_routine(pv,deriv,elem(ie),div)      ! latlon vector -> scalar
             endif
             if (npdg>0) then
                call divergence_sphere_wk_routine(pv,deriv,elem(ie),ptens_dg(:,:,k,ie))      ! latlon vector -> scalar
             endif
...
!THIS IS A NEW VERSION OF LIM8, from 3D code
    ...
    enddo
    call limiter_optim_iter_full(ptens,sphweights,minp,maxp,ptens_mass,1,nlev)
...
!-------------------------------------------------------------------------------------
  ...
  subroutine advance_hypervis(edge3,elem,hybrid,deriv,vtens,ptens,nt,nets,nete,dt2)
    ...
    use hybrid_mod, only : hybrid_t
    use element_mod, only : element_t
    use derivative_mod, only : derivative_t, laplace_sphere_wk_routine, vlaplace_sphere_wk_routine
       ...
       do ic=1,hypervis_subcycle
          do ie=nets,nete
             ...
             do k=1,nlev
                ...
                call laplace_sphere_wk_routine(ptot,deriv,elem(ie),var_coef=.false.,laplace=lap_p)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    if (hypervis_order == 2) then
       do ic=1,hypervis_subcycle
          call biharmonic_wk(elem,ptens,vtens,deriv,edge3,hybrid,nt,nets,nete,1,nlev)
...
!-----------------------------------------------------------------------------------
  ...
  subroutine advance_si_nonstag(elem, edge1,edge2,   edge3 ,  red     ,            &
    ...
    !    use parallel_mod
    ! ---------------------
    use derivative_mod, only : derivative_t,  gradient_wk_routine, divergence_routine
    use derivative_mod, only : vorticity_routine, gradient_routine
    ! ---------------------
    use time_mod, only : timelevel_t, smooth
    ! ---------------------
...
!    real (kind=real_kind) :: et,st
    ...
    call t_startf('advance_si_nonstag', t_detail_high)
...
#ifdef _WK_GRAD
          call gradient_wk_routine(elem(ie)%state%p(:,:,k,nm1),deriv,gradpm1(:,:,:))
          gradpm1(:,:,:)=gradpm1(:,:,:)*rrearth
          call gradient_wk_routine(elem(ie)%state%p(:,:,k,n0), deriv,  gradp(:,:,:))
          gradp(:,:,:)  =gradp(:,:,:)*rrearth
#else
          call gradient_routine(elem(ie)%state%p(:,:,k,nm1),deriv,gradpm1(:,:,:))
          gradpm1(:,:,:)=gradpm1(:,:,:)*rrearth
          call gradient_routine(elem(ie)%state%p(:,:,k,n0), deriv,  gradp(:,:,:))
          gradp(:,:,:)  =gradp(:,:,:)*rrearth
! ---
#endif
! --- endif _WK_GRAD
          ...
          call vorticity_routine(vco,deriv,zeta(:,:))
          zeta(:,:)=zeta(:,:)*rrearth
          ...
          !
          ! ==============================================
          call gradient_routine(E,deriv,grade(:,:,:))
          grade(:,:,:)=grade(:,:,:)*rrearth
...
!make sure its consistent lat lon NOT not contra or co-variant
    ...
    do ie=nets,nete
       ...
       do k=1,nlev
          ...
          call divergence_routine(gv  ,deriv,  div)
          div = div*rrearth
          call divergence_routine(gvm1,deriv,divm1)
          divm1 = divm1*rrearth
...
#ifdef _WK_GRAD
          call gradient_wk_routine(dp(:,:,k,ie),deriv,grad_dp(:,:,:,k,ie))
          grad_dp(:,:,:,k,ie)=grad_dp(:,:,:,k,ie)*rrearth
#else
          call gradient_routine(dp(:,:,k,ie),deriv,grad_dp(:,:,:,k,ie))
          grad_dp(:,:,:,k,ie)=grad_dp(:,:,:,k,ie)*rrearth
! ---
#endif
! --- endif _WK_GRAD
    ...
    call t_stopf('advance_si_nonstag', t_detail_high)
...
!----------------------------------------------------------------------------------------
  ...
  use hybrid_mod, only : hybrid_t
  use element_mod, only : element_t
  use derivative_mod, only : derivative_t, divergence_sphere_routine
  use derivative_mod, only : gradient_sphere_routine, vorticity_sphere_routine
  use edge_mod, only : edgevpack, edgevunpack
  use edgetype_mod, only : EdgeBuffer_t
  use bndry_mod, only : bndry_exchangev
  implicit none
  ...
  call t_startf('compute_and_apply_rhs', t_detail_high)

  call t_startf('construct_vtend_vgradp', t_detail_high)
  ! ===================================
  ! construct v tendencies and v.grad(p)
  ! on the velocity grid...
  ! ===================================
  ...
  call t_stopf('construct_vtend_vgradp', t_detail_high)
  
  
  call t_startf('in_caar bndry_exch', t_detail_high)
  call bndry_exchangeV(hybrid,edge3)
  call t_stopf('in_caar bndry_exch', t_detail_high)
  
  call t_startf('update_v_and_p', t_detail_high)
  do ie=nets,nete
     rspheremp     => elem(ie)%rspheremp
  ...
  call t_stopf('update_v_and_p', t_detail_high)
  call t_stopf('compute_and_apply_rhs', t_detail_high)
