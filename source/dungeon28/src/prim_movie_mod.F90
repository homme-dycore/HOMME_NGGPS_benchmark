...
contains
  ...
  subroutine prim_movie_init(elem, fvm, par, hvcoord,tl)
    use hybvcoord_mod, only : hvcoord_t
    ...
    use fvm_control_volume_mod, only : fvm_mesh_ari, fvm_struct
    ...
    do ios=1,max_output_streams
       if((output_frequency(ios) .gt. 0) ) then
          ...
          if(  (nf_selectedvar('phys_lat', output_varnames))) then
             if (ntrac==0) call abortmp('Error: to output phys_lat nc must be > 0')
             jj=0
             do ie=1,nelemd
                ii=0
                do j=1,nc
          ...
          if( (nf_selectedvar('phys_area', output_varnames))) then
             if (ntrac==0) call abortmp('Error: to output phys_area nc must be > 0')
             jj=0
             do ie=1,nelemd
                ii=0
                do j=1,nc
          ...
          if( (nf_selectedvar('phys_cv_lat', output_varnames))) then
             if (ntrac==0) call abortmp('Error: to output phys_cv_lat nc must be > 0')
             ...
             do ie=1,nelemd
                ...
                do j=1,nc
                   do i=1,nc
                      ...
                      jj=jj+1
                      ! take center of mass of elem(ie)%
                      var1(jj,1) = fvm(ie)%asphere(i,j)%lat
                      var2(jj,1) = fvm(ie)%asphere(i,j)%lon
                      var1(jj,2) = fvm(ie)%asphere(i+1,j)%lat
                      var2(jj,2) = fvm(ie)%asphere(i+1,j)%lon
                      var1(jj,3) = fvm(ie)%asphere(i+1,j+1)%lat
                      var2(jj,3) = fvm(ie)%asphere(i+1,j+1)%lon
                      var1(jj,4) = fvm(ie)%asphere(i,j+1)%lat
                      var2(jj,4) = fvm(ie)%asphere(i,j+1)%lon
                   end do
                end do
             end do
             var1=var1*180/dd_pi
...
!
  ...
  subroutine prim_movie_output(elem, tl, hvcoord, hybrid, nets,nete, fvm)
    ...
    use netcdf_io_mod, only : iodesc2d_nc,iodesc3d_nc
    ...
    real (kind=real_kind) :: varphys2D(nc*nc*nelemd)
    ...
    integer :: ncnt
    call t_startf('prim_movie_output:pio', t_detail_high)
#ifdef _AIX
!    call unbind()
#endif
       ...
       if((output_frequency(ios) .gt. 0)) then
          if ((output_start_time(ios) .le. tl%nstep) .and. &
...
#ifdef _PRIM
             if (nf_selectedvar('cly', output_varnames).or.nf_selectedvar('dp3d', output_varnames).or.&
             ...
             if(nf_selectedvar('dp3d', output_varnames)) then
                if (hybrid%masterthread) print *,'writing dp3d...'
                st=1
                do ie=1,nelemd
                   en=st+elem(ie)%idxp%NumUniquePts-1
                   call UniquePoints(elem(ie)%idxP,nlev,elem(ie)%state%dp3d(:,:,:,tl%n0),var3d(st:en,:))
                   st=en+1
                enddo
             ...
             if(nf_selectedvar('cly', output_varnames)) then
                if (hybrid%masterthread) print *,'writing cly'
                st=1
                do ie=1,nelemd
                   do j=1,np
                      do i=1,np
                         vartmp(i,j,1) = SUM(elem(ie)%state%dp3d(i,j,:,tl%n0)*&
                              (elem(ie)%state%Q(i,j,:,2)+2.0D0*elem(ie)%state%Q(i,j,:,3)))/&
                              SUM(elem(ie)%state%dp3d(i,j,:,tl%n0))
                      end do
                   end do
                   en=st+elem(ie)%idxp%NumUniquePts-1
                   call UniquePoints(elem(ie)%idxP,vartmp(:,:,1),var2d(st:en))
                   st=en+1
                enddo
                call nf_put_var(ncdf(ios),var2d,start2d,count2d,name='cly')
             end if
             if(nf_selectedvar('cl', output_varnames)) then
                if (hybrid%masterthread) print *,'writing cl'
                st=1
                do ie=1,nelemd
                   do j=1,np
                      do i=1,np
                   ...
                   st=en+1
                enddo
                call nf_put_var(ncdf(ios),var2d,start2d,count2d,name='cl')
             end if
             if(nf_selectedvar('cl2', output_varnames)) then
                if (hybrid%masterthread) print *,'writing cl2'
                st=1
                do ie=1,nelemd
                   do j=1,np
                      do i=1,np
                   ...
                   st=en+1
                enddo
                call nf_put_var(ncdf(ios),var2d,start2d,count2d,name='cl2')
             end if
#endif
             if (present(fvm)) then
             do qindex=1,min(ntrac,7)
                   ...
                   do k=1,nlev
                      ...
                      do ie=1,nelemd
                         do j=1,nc
                            do i=1,nc
                               jj=jj+1
                               varphys(jj,k)= fvm(ie)%c(i,j,k,qindex,n0_fvm)
                            end do
                         end do
                      end do
                   end do
             ...
             if(nf_selectedvar('psC', output_varnames)) then
                if (hybrid%masterthread) print *,'writing psC'
                jj=0
                do ie=1,nelemd
                   do j=1,nc
                      do i=1,nc
                         jj=jj+1
                         varphys2D(jj)= fvm(ie)%psc(i,j)
                      end do
                   end do
                end do
                call nf_put_var(ncdf(ios),varphys2D,start2d,count2d, name='psC',iodescin=iodesc2d_nc)
             endif

             if(nf_selectedvar('dp_fvm', output_varnames)) then
                if (hybrid%masterthread) print *,'writing dp_fvm'
                do k=1,nlev
                   jj=0
                   do ie=1,nelemd
                      do j=1,nc
                         do i=1,nc
                            jj=jj+1
                            varphys(jj,k)= fvm(ie)%dp_fvm(i,j,k,n0_fvm)
                         end do
                      end do
                   end do
                end do
                call nf_put_var(ncdf(ios),varphys,start, count, name='dp_fvm',iodescin=iodesc3d_nc)
             endif
             !
             ! For terminator chemistry: 
             !
             ...
             if(nf_selectedvar('cly_fvm', output_varnames)) then
                ...
                do ie=1,nelemd
                   do j=1,nc
                      do i=1,nc
                         jj=jj+1
                         varphys2D(jj)= SUM(fvm(ie)%dp_fvm(i,j,:,n0_fvm)*(&
                              fvm(ie)%c(i,j,:,2,n0_fvm)+2.0D0*fvm(ie)%c(i,j,:,3,n0_fvm)))/&
                              SUM(fvm(ie)%dp_fvm(i,j,:,n0_fvm))
                      end do
                   end do
                end do
                call nf_put_var(ncdf(ios),varphys2D,start2d,count2d, name='cly_fvm',iodescin=iodesc2d_nc)
             endif
             if(nf_selectedvar('cl_fvm', output_varnames)) then
                if (ntrac<3) call abortmp('Error: to output cl_fvm ntrac needs to >=3')
                ...
                call nf_put_var(ncdf(ios),varphys2D,start2d,count2d, name='cl_fvm',iodescin=iodesc2d_nc)
             endif
             if(nf_selectedvar('cl2_fvm', output_varnames)) then
                ...
                call nf_put_var(ncdf(ios),varphys2D,start2d,count2d, name='cl2_fvm',iodescin=iodesc2d_nc)
             endif
#endif
          end if!done fvm
             ...
             if(nf_selectedvar('omega', output_varnames)) then
                ...
                do ie=1,nelemd
                   en=st+elem(ie)%idxp%NumUniquePts-1
                   call UniquePoints(elem(ie)%idxp,nlev,elem(ie)%derived%omega,var3d(st:en,:))
                   st=en+1
                end do
...
!    call rebind()
#endif
    call t_stopf('prim_movie_output:pio', t_detail_high)
