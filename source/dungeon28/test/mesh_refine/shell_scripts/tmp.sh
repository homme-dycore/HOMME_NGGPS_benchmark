...
set mesh = ${NE}_${NEfine}
if ($ref != 1) then
		set mesh = north_${mesh}
	if ($smth != 0) then
...
set CFG="--enable-lapack --enable-blas --enable-mesh-refine PLEV=20 NP=4 --with-netcdf=$NETCDF_PATH --with-pnetcdf=$PNETCDF_PATH --enable-pio"
cd $src
