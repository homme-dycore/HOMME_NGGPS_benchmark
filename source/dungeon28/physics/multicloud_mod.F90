module multicloud_mod
  ...
  use derivative_mod, only     : gradient_routine, divergence_routine
...
!       rdy=2.0_real_kind/(elem(ie)%dy*rearth) ! strong derivative inverse y length
       ...
       call divergence_routine(gv,deriv,divpert(:,:))
       call divergence_routine(gubar,deriv,divubar(:,:))
       call gradient_routine(elem_physics(ie)%qmc(:,:,n0), deriv, gradqmc(:,:,:))
       call gradient_routine(elem_physics(ie)%teb(:,:,n0), deriv, gradteb(:,:,:))
...
!       rdy=2.0_real_kind/(elem(ie)%dy*rearth) ! strong derivative inverse y length
       ...
       call gradient_routine(elem_physics(ie)%qmc(:,:,np1),deriv,gradqmc_tmp(:,:,:))
       call gradient_routine(elem_physics(ie)%teb(:,:,np1),deriv,gradteb_tmp(:,:,:))
...
!       rdy=2.0_real_kind/(elem(ie)%dy*rearth) ! strong derivative inverse y length
       ...
       call divergence_routine(grad_qmc_np1(:,:,:,ie),deriv,divqmc_tmp(:,:))
       call divergence_routine(grad_teb_np1(:,:,:,ie),deriv,divteb_tmp(:,:))
