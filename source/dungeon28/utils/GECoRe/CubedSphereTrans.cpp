...
void CubedSphereTrans::RLLFromXYP(
	...
	// Correct lon to be on the interval [0, 2 pi]
	lon = fmod(lon + 2.0 * M_PI, 2.0 * M_PI);
}
...
void CubedSphereTrans::DerivTransXYPFromABP(
	double dX,
	double dY,
	double &dDxU,
	double &dDyU,
	...
	dDxxU = -2.0 * dX * (1.0 + dX * dX) * dDxU + dDxxU;
	dDxxU /= (2.0 * (1.0 + dX * dX) * (1.0 + dX * dX));

	dDxyU /= (1.0 + dX * dX) * (1.0 + dY * dY);

	dDyyU = -2.0 * dY * (1.0 + dY * dY) * dDyU + dDyyU;
	dDyyU /= (2.0 * (1.0 + dY * dY) * (1.0 + dY * dY));
}
