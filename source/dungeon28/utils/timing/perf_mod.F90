...
!========================================================================
!
   subroutine t_startf(event, timing_detail, handle)
!----------------------------------------------------------------------- 
...
!
   ...
   integer(shr_kind_i8), intent(in) :: timing_detail 
...
!-----------------------------------------------------------------------
!
   if (timing_initialized .and. timing_disable_depth .eq. 0) then
     if(timing_detail .le. timing_detail_limit) then
...
!========================================================================
!
   subroutine t_stopf(event, timing_detail, handle)
!----------------------------------------------------------------------- 
...
!
   ...
   call t_startf("t_prf", t_detail_max)
!$OMP MASTER
...
!$OMP END MASTER
   call t_stopf("t_prf", t_detail_max)
...
!========================================================================
!
   subroutine t_initf(LogPrint, LogUnit, mpicom, MasterTask, maxthreads) 
!----------------------------------------------------------------------- 
...
!$OMP MASTER
    ...
    if ( MasterTask2 ) then
       ...
       ! Read in the prof_inparm namelist from input.nl if it exists

#if defined(OSF1) || defined(_BGL) || defined(_NAMELIST_FROM_FILE)
       write(p_logunit,*) '(t_initf) Read in prof inparm namelist from input.nl '
       unitn = shr_file_getUnit()

       ierr = 1
       open( unitn, file=input.nl, status='old', iostat=ierr )
       if (ierr .eq. 0) then
...
#else
       write(p_logunit,*) '(t_initf) Read in prof_inparm namelist from standard input' 
       read(*,nml=prof_inparm)
       !rewind(5)
#endif
    endif
    ...
    if (perf_papi_enable) then
       ...
       if ( MasterTask2 ) then
          ...
          ! Read in the papi_inparm namelist from inout.nl if it exists

#if defined(OSF1) || defined(_BGL) || defined(_NAMELIST_FROM_FILE)
          write(p_logunit,*) '(t_initf) Read in papi_inparm namelist from input.nl'
          unitn = shr_file_getUnit()
          ...
          call shr_file_freeUnit( unitn )
#else
          write(p_logunit,*) '(t_initf) Read in papi_inparm namelist from standard input'
          read(*,nml=papi_inparm)
#endif
